...
## T-09 review — Test Engineer

Reviewed by: Test Automation Engineer critic (HOÀI NGHI — cố phá suite, không duyệt "có test là đủ"). REVIEW ONLY — không sửa code.
Date: 2026-08-04.
Scope: 8 test files mới + additions (loadtest-format, loadtest.store, loadtest-prefs, auth.store, loadtest-auth.store, api, env, loadtest-api, loadtest-auth-storage) + `loadtest-prefs.ts` (loadPrefs extraction) + `vitest.workspace.ts` + coverage thresholds 70%. Đã chạy thật: `npm run test` (**281/281 pass, 26 files**), `npm run test:coverage` (281 pass + coverage report), `npm run loadtest:test` (**180/180 pass, 15 files**).

### Findings

| Severity | Finding | Evidence | Verdict | Fix |
|---|---|---|---|---|
| MINOR | **`npm run test:coverage` chạy CẢ loadtest suite — workspace vẫn active dù script gọi `--config vitest.config.ts`**: coverage run chạy 281 tests (101 frontend + 180 loadtest, output có prefix `|loadtest|`), KHÔNG phải frontend-only như ý định comment (`vitest.config.ts` nói "The loadtest suite ... is run via the loadtest:test script"). Hệ quả: coverage script phụ thuộc Postgres loadtest (test chạy thật trên DB — thấy `[lt][db] query fail (23505)` warning) — DB down → `test:coverage` fail dù frontend coverage vẫn đạt. Coverage computation vẫn scoped ĐÚNG (report chỉ 2 file). | package.json `test:coverage` = `vitest run --config vitest.config.ts --coverage`; output `|loadtest| loadtest/__tests__/api-server.test.ts` trong coverage run; vitest.workspace.ts:1-15 | CONFIRMED (intent-mismatch) | Dùng `--project` (cách ly) hoặc chấp nhận + ghi rõ dependency |
| INFO | **Margin threshold mỏng — loadtest.store.ts functions 72.72% vs 70%**: 3 function không cover: `pauseRun`, `resumeRun`, `useLoadtestPoll`. Branches 74.07% cũng sát ngưỡng. Chỉ cần 1 test removal/refactor nhỏ là trip gate. | coverage-summary.json (store: funcs 8/11 = 72.72%); coverage/store/loadtest.store.ts.html `fstat-no` ×3 | CONFIRMED (coverage gap) | Thêm 2 test nhỏ pauseRun/resumeRun (best-effort wrapper) + 1 test useLoadtestPoll hook |
| INFO | **pauseRun/resumeRun hoàn toàn không test** — 2 action best-effort (catch im lặng) không có assert nào trong loadtest.store.test.ts. | loadtest.store.test.ts (run lifecycle: chỉ loadConfig/startRun/stopRun/pollOnce/resetRun/setProfile) | CONFIRMED | Sucess → gọi api; failure → không throw + state giữ nguyên |
| INFO | **`npm run test` giờ chạy cả 2 project → cần Postgres DB** (test loadtest chạy thật, không skip). Trước T-09 `npm run test` chỉ frontend. Thay đổi có chủ đích (workspace) nhưng cần note cho CI/local. | `npm run test` = `vitest run` (workspace); output api-server.test.ts (29 tests) chạy thật | PLAUSIBLE (đã chủ đích) | Chấp nhận hoặc tách nhanh frontend-only |

### Verify PASS

- **Không vacuous**: grep toàn bộ `src/` — không có `expect(true)`/`toBe(true)` đơn lẻ; mọi `toHaveBeenCalled(With)` đều đi kèm assert hành vi/state. loadtest-format.test.ts assert giá trị CỤ THỂ (vd `fmtCompact(8_200_000)==='8.2M'`, `fmtMs(1500)==='1.5s'`, `fmtClock(86399)==='23:59:59'`), không "tồn tại".
- **loadtest-format.test.ts edge cases**: 0, âm, NaN, Infinity, undefined/null, giá trị lớn đều có test cho fmtNum/fmtCompact/fmtMs/fmtClock/ticks/date/range.
- **loadtest-prefs.test.ts**: JSON hỏng (`'{'`, `'not json'`), sai kiểu (`"false"` string, `1`/`0` number), thiếu field (`{}`, `{"other":1}`), non-object (`[]`, `null`, `"str"`, `42`) → default. L-7 string-truthy bug có test hồi quy. localStorage roundtrip + corrupt → default không throw.
- **auth.store.test.ts**: mock chatApi/decodeJwt/tokenStorage đầy đủ; hydrate (no token/expired/valid/undecodable), login (success/2fa/no-accessToken/ApiError 401), logout. Refresh path không phải trách nhiệm store (interceptor — phủ ở api.test.ts).
- **loadtest-auth.store.test.ts**: mock loadtestApi + storage; hydrate (no session/expired/valid + authMe refresh/initialized guard) + login success/failure + logout (server error → vẫn clear client-side) + clearSession.
- **api.test.ts + loadtest-api.test.ts**: unwrap (có/không envelope), toApiError error envelope shape (statusCode/code/message/traceId/raw, instanceof ApiError, network→NETWORK, client→CLIENT), retryAfterSec 3 nhánh: body `retryAfterSec:30`→30, header `Retry-After:'15'`→15, thiếu/0→undefined.
- **loadPrefs extraction = pure refactor**: grep xác nhận JSON.parse duy nhất nằm trong `loadtest-prefs.ts` (parser); store chỉ dùng `loadPrefs()`/`savePrefs()` (loadtest.store.ts:80,83). Behavior giữ nguyên (init từ localStorage, setRequireEnvConfirm roundtrip).
- **Coverage honesty**: report CHỈ 2 file — loadtest-format.ts **100%** statements/branches/functions/lines; loadtest.store.ts **89.83%** stmts, **74.07%** branches, **72.72%** funcs, **89.83%** lines. Threshold 70% set trong vitest.config.ts (coverage.thresholds) và ENFORCE (vitest fail nếu dưới) — run pass, mọi metric > 70%.
- **Workspace**: `npm run test` = 281 (26 files) = 101 frontend + 180 loadtest; `npm run loadtest:test` chỉ 180 (15 files); include không overlap (`src/**/*.test.{ts,tsx}` vs `loadtest/__tests__/**/*.test.ts`) → không test file nào chạy 2 lần trong 1 lệnh.

### Verdict

**AN TOÀN ĐỂ COMMIT — không blocker.** 281/281 tests xanh (chạy thật), coverage 2 file mục tiêu vượt threshold 70% (format 100%, store 89.83% lines / 72.72% funcs) và threshold được enforce; test không vacuous — phủ edge case (0/âm/NaN/large/malformed/wrong-type/missing) và assert giá trị cụ thể; loadPrefs extraction là pure refactor (không raw JSON.parse ngoài parser). 1 MINOR: coverage script chạy cả loadtest suite (workspace vẫn active dù `--config`) → phụ thuộc DB; 3 INFO: margin funcs mỏng (72.72%), pauseRun/resumeRun/useLoadtestPoll chưa test, `npm run test` cần DB. Không ảnh hưởng correctness — không cần fix trước commit.

## T-09 review — Code Review

Reviewed by: Code Reviewer critic (DEFAULT = HOÀI NGHI). REVIEW ONLY — không sửa code.
Date: 2026-08-04.
Scope: T-09 — extract `parseLoadtestPrefs` (src/store/loadtest-prefs.ts) + test infra (vitest.workspace.ts, vitest.config.ts, loadtest/vitest.config.ts, package.json scripts, README session docs). Đã chạy thật: `npm run test` (281/281 pass, 26 files), `npm run test:coverage` (coverage trên 70%), `npm run loadtest:test` (180/180 pass, 15 files), `npm run typecheck`, `npm run build` — đều xanh.

### Findings

| # | Sev | Finding | Evidence | Verdict |
|---|---|---|---|---|
| 1 | INFO | **Behavior preservation — valid data identical.** OLD `loadPrefs` (HEAD `loadtest.store.ts:29-37`) `{ requireEnvConfirm: true, ...(JSON.parse(raw)) }` vs NEW `parseLoadtestPrefs` (loadtest-prefs.ts:17-29). Default `true`, field `requireEnvConfirm: boolean` — giống hệt. Với dữ liệu hợp lệ (boolean `true`/`false`) cả 2 return cùng giá trị — không thay đổi hành vi user. | diff HEAD:src/store/loadtest.store.ts | PASS |
| 2 | INFO | **Malformed-data handling — deliberate safe fix (L-7).** OLD nhận mọi type (string `"false"`, number `0`/`1`, `null`) và coerce truthy/falsy tại nơi dùng; NEW chỉ nhận boolean, else default `true`. Diff vô hại: `0`/`null`/`"false"`(string) OLD → dialog ẩn, NEW → dialog hiện (fail-safe — đúng ý định L-7, comment loadtest-prefs.ts:5-7). Không mất setting hợp lệ nào. | loadtest-prefs.ts:22-24; test loadtest-prefs.test.ts:26-34 | PASS |
| 3 | LOW | **Unknown fields bị drop (forward-compat).** OLD spread giữ lại field lạ trong object (`setRequireEnvConfirm` lưu `{...loadPrefs(), requireEnvConfirm: v}` → field lạ được bảo toàn qua save); NEW parse chỉ return `{requireEnvConfirm}` → field lạ bị xoá khi save tiếp. Hôm nay interface chỉ có 1 field, không impact user — nhưng app mới hơn ghi field thứ 2, build này toggle pref sẽ xoá field đó khỏi localStorage. | loadtest-prefs.ts:23; loadtest.store.ts:83 | MINOR (theo dõi) |
| 4 | LOW | **`test:coverage` chạy cả workspace, không chỉ frontend.** `vitest run --config vitest.config.ts --coverage` — khi `vitest.workspace.ts` tồn tại, `--config` bị ignore, workspace (loadtest + frontend) đều chạy (bằng chứng: 26 files / 281 tests, bằng đúng `npm run test`). Coverage report vẫn scope đúng 2 file (include) nên threshold đúng — nhưng script kéo thêm loadtest suite (lãng phí; loadtest fail cũng fail `test:coverage`). Không phải bug correctness. | package.json:17; vitest.workspace.ts:9-12 | MINOR |
| 5 | LOW | **Coverage gate không còn phủ prefs logic.** `loadtest-prefs.ts` (logic parse mới tách ra) nằm NGOÀI `coverage.include` (chỉ `loadtest-format.ts` + `loadtest.store.ts`). Có test đơn vị (10 case) nhưng gate G-1 không phát hiện regression coverage ở prefs. | vitest.config.ts:20-23 | MINOR (theo dõi) |
| 6 | PASS | **Workspace isolation đúng.** loadtest project: `environment:'node'`, include `loadtest/__tests__/**/*.test.ts`, root chat-app; frontend: `environment:'jsdom'`, include `src/**/*.test.{ts,tsx}`, setupFiles setup.ts. Không overlap → không double-run. Frontend chạy jsdom, loadtest chạy node — không lẫn env. | vitest.workspace.ts:9-11; loadtest/vitest.config.ts:5-9; vitest.config.ts:14-17 | PASS |
| 7 | PASS | **package.json scripts đúng.** `test`/`test:watch` → workspace; `loadtest:test` → `--config loadtest/vitest.config.ts` (CI — vẫn chạy, 180/180 green). | package.json:14-17 | PASS |
| 8 | PASS | **README chính xác.** `SESSION_TTL_MS = 12*60*60*1000` (auth.ts:15) ✓; banner ≤ 30 phút dismissible, text từ `expiresAt` không hardcode "12h" (loadtest-session.ts:18-22, session-expiry-banner.tsx) ✓; 401 → clearSession qua `onLoadtestAuthFailure` (loadtest-api.ts:91-93, loadtest-auth.store.ts:94-96) + guard redirect login ✓; key `loadtest.auth` ✓. | README.md:55-62; auth.ts:15; loadtest-session.ts:10 | PASS |
| 9 | PASS | **Coverage threshold hoạt động.** Thresholds 70% all-metrics trên 2 file; hiện tại 92.77% stmts / 86.27% branches / 85% funcs / 92.77% lines. Dưới 70% → vitest exit non-zero (G-1 gate). Lưu ý: `test:coverage` không nằm trong `build` — gate chỉ chặn nếu CI gọi script này. | vitest.config.ts:22-23; coverage report | PASS |
| 10 | PASS | **Commands xanh.** `npm run test` 281/281 (26 files); `npm run test:coverage` green; `npm run loadtest:test` 180/180 (15 files); `npm run typecheck` exit 0; `npm run build` exit 0 (chỉ chunk-size warning). | output thật | PASS |

### Verdict

**AN TOÀN ĐỂ COMMIT — không blocker.** Tách `parseLoadtestPrefs` không đổi contract cho dữ liệu hợp lệ (default/field/type giống hệt), xử lý malformed-data là fix L-7 có chủ đích (fail-safe), test đơn vị 10 case phủ đủ biên (null/empty/JSON hỏng/sai kiểu/thiếu field/non-object). Workspace Vitest cô lập đúng env (jsdom vs node), không double-run; scripts test/test:watch/loadtest:test đúng; README session 12h + banner chính xác so với code; coverage gate 70% đạt và đủ mạnh để chặn regression. 3 MINOR/INFO theo dõi, không chặn: (1) drop unknown fields — forward-compat hẹp, chưa có field thứ 2; (2) `test:coverage` thực tế chạy cả workspace (--config bị ignore khi có workspace file) — nên sửa thành `vitest run --project` hoặc nhận biết; (3) coverage gate không phủ `loadtest-prefs.ts` — nên thêm file vào `coverage.include`.

## T-08 review — AppSec

Reviewed by: Application Security Engineer (HOÀI NGHI — REVIEW ONLY, không sửa code).
Date: 2026-08-04.
Scope: CSP (vite.config.ts injectCspMeta + PROD_CSP_BASE), socket token removal (src/lib/socket.ts + loadtest/socket-farm.ts), ErrorBoundary, DEBUG-LOGIN removal, localStorage token (Q-4). Đã verify dist/index.html (build thật) + engine.io-client source (node_modules) + gateway websocket.gateway.ts:147-149.

### Findings

| # | Severity | Finding | Evidence | Verdict |
|---|---|---|---|---|
| 1 | **HIGH** | **connect-src thiếu default gateway origin khi VITE_GATEWAY_URL không set → socket bị CSP chặn.** `injectCspMeta` chỉ thêm origin gateway khi `VITE_GATEWAY_URL` set (vite.config.ts:27-35), nhưng `env.ts:7` default `http://localhost:3000`. Build thật `dist/index.html:4` (KHÔNG set VITE_GATEWAY_URL) → `connect-src 'self' https://fonts.googleapis.com https://fonts.gstatic.com` — KHÔNG có `http://localhost:3000`/`ws://localhost:3000` → socket.io polling XHR + websocket upgrade tới gateway đều bị CSP chặn → chat không kết nối được. `.env.example:3` có set var nên build đúng config thì không dính → không phải Critical mọi build, nhưng default build chắc chắn broken. **Fix: inject cùng default như env.ts khi unset, hoặc document bắt buộc build với VITE_GATEWAY_URL.** | vite.config.ts:27-35, env.ts:7, dist/index.html:4 | **BLOCKER bắt buộc fix trước khi dùng default build** |
| 2 | **MEDIUM** | **Browser websocket transport KHÔNG gửi được `extraHeaders` → mỗi connect bắt đầu bằng 1 attempt websocket không token (fail auth) rồi mới fallback polling.** `socket.ts:89` dùng `extraHeaders` + `transports:['websocket','polling']`. WebSocket API browser không cho set custom header (engine.io-client `websocket.js:128-129` browser path `new WebSocketCtor(uri)` — header bị drop). Gateway `websocket.gateway.ts:147-149` đọc query.token → header → cả 2 rỗng → "Missing token → disconnect" → client fallback polling (XHR có header) → auth OK → upgrade websocket tái dùng sid (không re-auth). Kết quả: hoạt động nhưng mỗi reconnect log fail + `wsConnections.inc()` + latency. Loadtest (Node, ws lib hỗ trợ header) không dính. Token removal khỏi URL: **CONFIRMED** cả 2 file — không còn token trong query. | socket.ts:89, loadtest/socket-farm.ts:99, node_modules/engine.io-client/.../websocket.js:128-129, gateway websocket.gateway.ts:147-149 | **Theo dõi — cân nhắc chuyển `transports:['polling']` hoặc dùng `auth` (chấp nhận query) nếu cần real websocket** |
| 3 | **LOW** | **`dangerouslySetInnerHTML` ở MatchingScreen.tsx:131 — không phải sink XSS hiện tại** (search chỉ nhận hardcoded string/DEMO_PEOPLE constant, không phải user input), nhưng THREAT-MODEL.md:40 ghi "T-08 eslint `react/no-danger`" — **project KHÔNG có eslint config** (không có eslint dependency, không .eslintrc) → rule không được enforce. Nếu sau này `search` nhận user-controlled string → sink. MessageBubble render user content dạng text (React-escape) — không dangerouslySetInnerHTML trên user data. | MatchingScreen.tsx:43,53,73,131, package.json, THREAT-MODEL.md:40 | **Theo dõi — thêm eslint react/no-danger hoặc bỏ claim trong THREAT-MODEL** |
| 4 | INFO | `style-src 'unsafe-inline'` — chấp nhận được (React inline style prop ở MatchingScreen:118,147 cần). Không có user-controlled style injection path (user content là text node). | dist/index.html:4 | PASS |

### Verify PASS

| Hạng mục | Kết quả |
|---|---|
| `script-src 'self'` + injectTo head-prepend | PASS — dist/index.html:4 meta CSP là tag đầu tiên trong `<head>` (trước mọi script), chỉ 1 module script external `/assets/index-DZe2p7vB.js`, không inline script. |
| Socket token không còn trong URL query (cả 2 file) | PASS — socket.ts:89 + socket-farm.ts:99 chỉ `extraHeaders Authorization Bearer`; token chỉ đi qua header (XHR polling) hoặc ws lib (Node). Không proxy/access-log nào log Authorization header mặc định. |
| ErrorBoundary không leak PII prod | PASS — ErrorBoundary.tsx:74-79 details chỉ hiện khi `import.meta.env.DEV`; prod console.error chỉ `{ name }` (line 109); onError không được wire tới remote reporting (App.tsx:51, app-shell.tsx:306 không truyền onError). |
| DEBUG-LOGIN removal | PASS — grep `DEBUG-LOGIN|DEBUG_LOGIN` trong src/ = 0 match. |
| console.log sensitive data | PASS — grep `console.(log|error)(token|password|accessToken|deviceInfo|Authorization|Bearer)` = 0 match. socket.ts log payload (displayName, roomId, content) — không token/credential. |
| localStorage residual (Q-4) | PASS — THREAT-MODEL.md:40 document rõ residual (CSP chặn exfil + admin token 12h bounded + no dangerouslySetInnerHTML user content). Loadtest token lưu localStorage `loadtest.auth` (loadtest-auth-storage.ts:8) — đúng như THREAT-MODEL mô tả. |

### Verdict

**COMMIT ĐƯỢC — 1 HIGH bắt buộc fix trước khi dùng default build.** Finding 1 (connect-src thiếu default origin) là gap logic CSP vs env.ts default — build không set `VITE_GATEWAY_URL` thì socket bị CSP chặn toàn bộ. `.env.example` đã set var nên build đúng config không dính; fix nhỏ (inject cùng default origin khi unset). Finding 2 (websocket browser không gửi header) — hoạt động nhờ fallback polling + upgrade reuses sid, không leak token, nhưng mỗi connect có 1 attempt fail auth ở gateway; cân nhắc `transports:['polling']` hoặc chấp nhận query `auth`. Finding 3 (no eslint) — doc over-claim, không phải bug thật. Các mục còn lại PASS sạch: CSP đúng nhất (script-src 'self', head-prepend, không wildcard ws/wss), token không còn trong URL, ErrorBoundary không leak prod, DEBUG-LOGIN đã xoá, Q-4 residual được document.

## T-08 review — Socket

Reviewed by: Realtime Collaboration Engineer (DEFAULT = HOÀI NGHI). REVIEW ONLY — không sửa code.
Date: 2026-08-04.
Scope: T-08 — bỏ `query: { token }` khỏi chat socket (src/lib/socket.ts) + loadtest socket-farm (loadtest/socket-farm.ts); chỉ còn `extraHeaders: { Authorization: Bearer }`. Đã đọc gateway-auth-service/src/infrastructure/driving-adapters/websocket/gateway/websocket.gateway.ts:144-165, engine.io-client v6 source (node_modules), chạy `npm run loadtest:test` (180/180 pass, 15 files) + `npx vitest run src/lib/socket.test.ts` (1/1 pass).

### Findings

| # | Severity | Finding | Evidence (file:line) | Verdict |
|---|---|---|---|---|
| 1 | MEDIUM | **Browser websocket transport KHÔNG gửi Authorization header.** `extraHeaders` chỉ được engine.io-client gửi cho (a) polling trong browser, (b) websocket trong Node (`ws` package). Browser build dùng `new WebSocket(uri)`/`new WebSocket(uri, protocols)` — bỏ `opts.headers` (chỉ React Native truyền opts). Package.json `browser` field map `websocket.node.js`→`websocket.js`. | chat-app/node_modules/engine.io-client/build/esm/transports/websocket.js `WS.createSocket` (không truyền opts khi non-ReactNative); node_modules/engine.io-client/package.json `"browser"`; docs socket.io: extraHeaders hoạt động cho polling trong browser | CONFIRMED — regression |
| 2 | MEDIUM | **Hệ quả: chat-app (browser) mất websocket transport, lặng lẽ fallback polling.** Kết nối: thử websocket → gateway handleConnection đọc `query?.token` (undefined) rồi `headers?.authorization` (undefined — browser không gửi header) → `client.disconnect()` → engine.io fallback polling → polling XHR GỬI được header → CORS preflight (gateway main.ts:26 allow Authorization + socket.io cors reflect) → kết nối thành công qua polling. App vẫn chạy nhưng websocket chết hoàn toàn; nếu chuyển `transports: ['websocket']` (như loadtest) → outage toàn bộ. Node/loadtest không bị ảnh hưởng (dùng `ws` package, header gửi qua upgrade). | src/lib/socket.ts:84-89 (transports websocket+polling, extraHeaders); websocket.gateway.ts:147-149 (query trước, fallback header); gateway main.ts:23-28 (CORS allow Authorization) | CONFIRMED — regression |
| 3 | MEDIUM | **Fix đề xuất phải đổi cả 2 phía.** Cách socket.io-classic là `auth: { token }` (gửi trong CONNECT packet payload, chạy mọi transport kể cả browser websocket) — nhưng gateway hiện KHÔNG đọc `handshake.auth` (chỉ query + headers). Fix: client thêm `auth: { token }` + gateway thêm `handshake.auth?.token` vào dòng 147-149 (ưu tiên auth/header). | websocket.gateway.ts:147-149 (chỉ đọc query?.token || headers?.authorization); socket.ts:89 (chỉ extraHeaders) | INFO — cần fix 2 phía |
| 4 | LOW | **Test gap: không có test nào chạy handshake thật với gateway.** `socket-farm.test.ts:11` và `src/lib/socket.test.ts:4` đều `vi.mock('socket.io-client')` — 180/180 chỉ assert `io()` được gọi với `extraHeaders.Authorization` + `query` undefined, KHÔNG validate gateway thật xác thực token qua header. T-11 contract test có thể bao phủ; nếu chưa, đây là lỗ hổng test. | loadtest/__tests__/socket-farm.test.ts:11,34-51; src/lib/socket.test.ts:4,25-35 | GAP |
| 5 | PASS | Reconnect: socket.io Manager giữ nguyên opts (extraHeaders) cho mỗi lần reconnect → header được gửi lại; loadtest re-join room qua `chat:join` trên 'connect'. Không bị ảnh hưởng bởi việc dời token. | socket.ts:91-95; socket-farm.ts:100-115 | PASS |
| 6 | PASS | Outbox/resend + clientMsgId dedupe hoàn toàn event-level (chat:send / chat:message echo), không phụ thuộc handshake token. Không bị ảnh hưởng. | socket.ts:194-226 | PASS |
| 7 | PASS | Thứ tự trích xuất gateway xác nhận đúng: `query?.token` trước, `headers?.authorization` sau. Client Node (loadtest) gửi header qua `ws` package → gateway xác thực được. | websocket.gateway.ts:147-149; node_modules/engine.io-client/build/esm/transports/websocket.node.js (truyền opts.headers vào `new ws.WebSocket`) | PASS |

### Verdict

**APPROVE-WITH-CONDITIONS — safe to commit, nhưng có 1 regression thật (không chặn).** Không có outage: loadtest (Node) và chat-app (browser qua polling fallback) đều kết nối được; SEC-3 đạt (token không còn trong query). Tuy nhiên browser websocket transport bị chết âm thầm (engine.io-client bỏ header trên native WebSocket) — app chạy bằng polling, degrades latency/điện báo, và nếu ai đặt `transports: ['websocket']` sẽ mất kết nối. Điều kiện: (1) TẠO TICKET fix = dùng `auth: { token }` ở client + gateway đọc `handshake.auth?.token` (đổi 2 phía); (2) T-11 contract test phải cover handshake thật với header (và sau fix là auth) — không dựa 180/180 unit test vì mock socket.io-client. Hiện tại commit OK; không phải hard blocker.

## T-08 review — Code Review

Reviewed by: Code Reviewer critic (DEFAULT = HOÀI NGHI — REVIEW ONLY, không sửa code).
Date: 2026-08-04.
Scope: T-08 frontend hardening — refresh endpoint (env.ts + api.ts), socket token removal (socket.ts + socket-farm.ts), ErrorBoundary (2-layer mount), CSP build plugin (vite.config.ts), loadtest-api retryAfterSec, allowRegister (types/loadtest.ts + Login/RegisterPage), 429 UX (ControlPanelPage + app-shell), session notice (loadtest-session.ts + session-expiry-banner.tsx). Đã build thật + grep dist/index.html, đọc gateway auth.controller.ts:314-325 + auth-token.service.ts:36-111 + websocket.gateway.ts:144-165, chạy `npm run build` / `typecheck` / `loadtest:typecheck` / `loadtest:test` (180/180) / `test` (281/281) — đều xanh.

### Findings

| # | Severity | Finding | Evidence | Verdict |
|---|---|---|---|---|
| 1 | **HIGH** | **CSP default gateway origin thiếu khi build không set `VITE_GATEWAY_URL` — socket bị chặn khi app serve khác origin.** Đã verify bằng build thật: build KHÔNG set var → `dist/index.html` connect-src = `'self' fonts` (KHÔNG có gateway); build với `VITE_GATEWAY_URL=https://gateway.mayogu.test` → connect-src có `https://gateway.mayogu.test`. `env.ts:7` default `http://localhost:3000` nhưng plugin chỉ inject khi var set (vite.config.ts:27-35). CSP scheme-matching: `http://localhost:3000` trong connect-src match `ws://localhost:3000` (http→ws/wss hợp lệ) — nên khi set var thì đúng. `.env.example:3` đã set var → build đúng config không dính. **Fix: inject cùng default origin khi unset (khớp env.ts), hoặc hard-fail build khi thiếu var.** | vite.config.ts:27-35, env.ts:7, dist/index.html (2 build thật) | **BLOCKER cho default build (đã biết — AppSec review T-08 f1)** |
| 2 | **MEDIUM** | **Browser websocket transport mất hẳn — chat socket chỉ chạy bằng polling.** `socket.ts:89` `extraHeaders` + `transports:['websocket','polling']`; WebSocket API browser không set được custom header → attempt websocket đầu tiên bị gateway `client.disconnect()` (websocket.gateway.ts:147-154: query?.token → headers?.authorization, cả 2 rỗng) → fallback polling (XHR có header) → auth OK. Upgrade websocket reuses sid (không re-auth). App hoạt động nhưng websocket chết âm thầm + mỗi connect 1 attempt fail + `wsConnections.inc()`. Loadtest (Node `ws` lib) không dính. **Fix (2 phía): client `auth:{token}` + gateway đọc `handshake.auth?.token`.** | socket.ts:84-89, websocket.gateway.ts:147-154, engine.io-client websocket.js (browser bỏ opts.headers) | CONFIRMED (đã biết — Socket review T-08 f1-3) |
| 3 | PASS | **Refresh contract đúng: `POST {gatewayUrl}/auth/refresh-token` body `{refreshToken}` → `{accessToken, refreshToken}`.** Endpoint public (không auth guard — auth.controller.ts:314-325, main.ts chỉ `whitelist:true`), dùng bare `axios.post` (không qua instance → không loop interceptor, không cần Authorization header). `unwrap` xử lý cả envelope `{success,data}` lẫn raw body. Lưu cả 2 token mới (`tokenStorage.set(access, data?.refreshToken ?? refresh)`). Test api.test.ts:81-99 cover happy path. | api.ts:57-72, 61; auth.controller.ts:314-325; RefreshTokenResponseDto | PASS |
| 4 | PASS | **Không infinite loop khi refresh.** `_retried` guard + `isRefreshCall` (url includes refreshEndpoint) + refresh dùng bare axios không qua response interceptor → 401 của chính refresh call không tái-refresh. Retry request 401 lần 2 → `_retried=true` → reject cho caller (không loop). Refresh fail → `tokenStorage.clear()` + `authFailureCb` → store logout → ProtectedRoute redirect login (silent logout — có chủ đích). | api.ts:74-92 | PASS |
| 5 | PASS | **CHAT và LOADTEST refresh độc lập đúng.** 2 axios client riêng (api.ts `instance` baseURL gateway; loadtest-api.ts `client` baseURL `/api/loadtest`), 2 storage riêng (tokenStorage vs loadtestAuthStorage). Loadtest không có refresh — 401 → `clearSession()` → guard redirect login (đúng thiết kế "different auth systems"). | api.ts:37-47, loadtest-api.ts:27,88-96, loadtest-auth.store.ts:94-96 | PASS |
| 6 | LOW | **Refresh call không có timeout (bare axios default timeout=0).** Nếu gateway treo ở refresh, mọi request 401 đang đợi chung `refreshing` promise đứng im vô hạn (instance chính có timeout 15s nhưng refresh không). **Fix: thêm `timeout: 15000` vào axios.post refresh.** | api.ts:61 | LOW |
| 7 | INFO | **Cross-tab race: refresh token single-use (server `atomicGetAndDeleteSession` — auth-token.service.ts:42-45) + localStorage chia sẻ → 2 tab expire cùng lúc thì 1 tab thắng, tab kia refresh fail → logout.** Inherent với single-use token; không phải regression T-08. | api.ts:57-72, auth-token.service.ts:42-52 | INFO (theo dõi) |
| 8 | PASS | **ErrorBoundary bắt render error đúng, prod không leak PII.** `getDerivedStateFromError` + `componentDidCatch`; prod chỉ `console.error({name})`, `<details>` chi tiết chỉ khi `import.meta.env.DEV`. Accessible: `role="alert"`, `tabIndex=-1`, nút reload `autoFocus`, focus container trong componentDidUpdate. resetKey=pathname (route-level) reset khi đổi trang. Không ảnh hưởng guard: ProtectedRoute/RequireLoadtestAuth render `<Navigate>` (không throw) nên không bị boundary nuốt. Fallback tự bảo vệ (try/catch → div tĩnh). Test ErrorBoundary.test.tsx:12-35 cover 2 case. | ErrorBoundary.tsx:44-154, App.tsx:51, app-shell.tsx:306, ProtectedRoute.tsx:17, require-auth.tsx:29-31 | PASS |
| 9 | LOW | **App-level boundary không resetKey → sau khi arm, không tự reset khi điều hướng (back/forward), chỉ reset khi reload hoặc bấm "Về trang chủ".** Có chủ đích (chống crash-loop) nhưng UX dai dẳng. | ErrorBoundary.tsx:113-123, App.tsx:51 | LOW (theo dõi) |
| 10 | PASS | **429 UX countdown đúng, không kẹt vĩnh viễn.** `startCooldown`/`stopCooldown` countdown 1s, effect dependency `[cooldown > 0]` → cleanup khi về 0 → nút re-enable. Stop button disabled `stopCooldown > 0` hiện "Thử lại sau Ns". `retryAfterSec` từ body hoặc `Retry-After` header (toApiError). Không nhánh nào set cooldown mà không đếm ngược. | ControlPanelPage.tsx:92-105,156-158,413-416; app-shell.tsx:43-49,60-63,105-108; loadtest-api.ts:59-68 | PASS |
| 11 | PASS | **Register CTA ẩn đúng khi allowRegister=false.** Backend `/config` trả `allowRegister` (settings.ts:30); type `LoadTestConfig.allowRegister` (types/loadtest.ts:80). LoginPage default true (safe), config lỗi → CTA giữ hiện (403 fallback vẫn chạy). RegisterPage config lỗi → `setConfigLoaded(true)` → form hiện (không chặn). | LoginPage.tsx:28-44,127-140; RegisterPage.tsx:39-63; settings.ts:30 | PASS |
| 12 | LOW | **Session text sai khi còn < 1 giờ.** `sessionExpiryText` `Math.round(rem/3600000)` + `Math.max(1,...)` → trong cửa sổ cảnh báo 30 phút luôn hiện "hết hạn trong 1 giờ" dù chỉ còn 5-29 phút. **Fix: hiện phút khi `rem < 60*60*1000`.** | loadtest-session.ts:25-27; session-expiry-banner.tsx:24-27 | LOW |
| 13 | PASS | **Session notice đúng:** `expiresAt` từ server (login set `res.expiresAt` — loadtest-auth.store.ts:60), `shouldWarnSession` chỉ true khi `0 < rem <= 30min`, dismissible per tab, re-check 60s. Khi hết hạn thật → 401 → clearSession → redirect. | loadtest-session.ts:18-22, session-expiry-banner.tsx:14-42 | PASS |
| 14 | PASS | **Builds/tests xanh:** `npm run build` exit 0 (chunk-size warning thôi), `typecheck` exit 0, `loadtest:typecheck` exit 0, `loadtest:test` 180/180 (15 files), `test` 281/281 (26 files). CSP meta inject head-prepend đúng (grep dist/index.html). | output thật | PASS |
| 15 | LOW | **Test gap refresh:** api.test.ts chỉ cover happy path (401 → refresh → retry); KHÔNG cover refresh-fail (clears + logout), retry-once (lần 2 không re-refresh), concurrent dedup, isRefreshCall guard. Socket handshake thật cũng không test (đã note Socket review f4). **Fix: thêm test cho nhánh fail + retry-once.** | api.test.ts:81-99 | GAP |

### Verdict

**SAFE TO COMMIT — không blocker mới.** Refresh flow đúng contract (endpoint public, body `{refreshToken}`, response `{accessToken,refreshToken}`, retry-once, không loop, độc lập CHAT/LOADTEST), ErrorBoundary chắc (bắt render error, không leak PII prod, accessible, không can thiệp guard), 429 UX đúng, register CTA đúng (safe default), session banner đúng (chỉ text phút bị lệch), 5 commands build/test đều xanh. 2 finding đã biết từ các review T-08 trước vẫn tồn tại: (1) HIGH — CSP default gateway origin thiếu khi build không set `VITE_GATEWAY_URL` (chỉ hại default build; `.env.example` đã set) — nên fix inject default; (2) MEDIUM — browser websocket transport chết âm thầm, chat chạy polling (fix 2 phía `auth` + gateway đọc `handshake.auth`). 3 LOW theo dõi: refresh không timeout, app-level boundary không tự reset, session text "<1h". 1 test gap: refresh fail/retry-once chưa cover. Không có correctness bug chặn commit.

## W3 Reality Check

Reviewed by: Reality Checker (DEFAULT = FAIL — cần bằng chứng quá thuyết phục). REVIEW ONLY, không sửa code.
Date: 2026-08-04.
Scope: T-08 frontend hardening + T-09 frontend tests. Tất cả lệnh chạy thật trên máy.

### Evidence

| # | Check | Requirement | Evidence (thật) | Status |
|---|---|---|---|---|
| 1 | Tests | 296 green | `npm run test`: 27 files, **296/296 passed** | PASS |
| 2 | Loadtest tests | 180 green | `npm run loadtest:test`: 15 files, **180/180 passed** | PASS |
| 3 | Typecheck | clean | `npm run typecheck` exit 0; `npm run loadtest:typecheck` exit 0 | PASS |
| 4 | Build | clean | `npm run build` exit 0 (chỉ chunk-size warning) | PASS |
| 5 | CSP (FIX-1) | connect-src chứa `http://localhost:3000` + fonts, KHÔNG ws:/wss:, script-src 'self', không inline script | dist/index.html: `connect-src 'self' https://fonts.googleapis.com https://fonts.gstatic.com http://localhost:3000`; `script-src 'self'`; `ws:/wss:` count = **0**; unsafe-inline chỉ có trong style-src (không phải script-src) | PASS |
| 6 | Socket auth client (FIX-2) | `auth` field + extraHeaders giữ, KHÔNG token trong query | src/lib/socket.ts:91-92 `auth: { token }, extraHeaders: { Authorization: Bearer ... }`; loadtest/socket-farm.ts:100-101 giống hệt; `query: { token` count trong src + loadtest = **0** | PASS |
| 7 | Socket auth gateway (FIX-2) | Thứ tự auth.token FIRST → query.token → headers.authorization; typecheck | websocket.gateway.ts:147-150: `handshake.auth?.token \|\| handshake.query?.token \|\| handshake.headers?.authorization` — đúng thứ tự; `npm run build` (nest build) exit 0 | PASS |
| 8 | Refresh timeout (FIX-3) | AbortController + timeout trong doRefresh | src/lib/api.ts:63-64: `new AbortController()` + `setTimeout(() => controller.abort(), REFRESH_TIMEOUT_MS)` | PASS |
| 9 | Banner text (FIX-4) | < 1h hiện phút | src/lib/loadtest-session.ts:30-32: `rem < 3600_000` → `Phiên loadtest hết hạn trong ${minutes} phút...` | PASS |
| 10 | Coverage (FIX-5) | 3 file ≥ 70% (stmts/branch/funcs/lines) | loadtest-format.ts: **100/100/100/100**; loadtest.store.ts: **100/78.78/100/100**; loadtest-prefs.ts: **95.65/91.66/100/95.65**; All files 99.47% stmts, 88.4% branch. Thresholds 70 cấu hình vitest.config.ts:23, không có threshold failure | PASS |
| 11 | No secrets | DEBUG-LOGIN trong src = 0 | grep `DEBUG-LOGIN` src/: **0 matches** | PASS |
| 12 | Git cleanliness | Chỉ file dự kiến; KHÔNG file gateway trong chat-app; không .env/data | `git status --short`: chỉ src/ loadtest/ docs/ package.json vite.config.ts vitest.config.ts .env.example (template — chỉ URL, không secret); không path nào của gateway-auth-service | PASS |

### Verdict

**PASS** — 12/12 xanh. Tất cả bằng chứng chạy thật: 296 + 180 test green, typecheck/build (cả chat-app lẫn gateway) exit 0, CSP đúng (gateway origin + fonts, không ws wildcard, không inline script), socket auth đủ 2 phía (client `auth` + gateway đọc auth.token trước query/header), refresh có AbortController timeout, banner hiện phút khi <1h, coverage 3 file loadtest đều vượt xa 70% (thấp nhất branch 78.78%), không secret, git status sạch.

Follow-up: thay đổi gateway (websocket.gateway.ts:147-150) nằm ở repo riêng `c:\MAYogu_VIASG\gateway-auth-service` — người dùng phải commit riêng ở đó; repo chat-app không chứa thay đổi gateway (đã xác nhận git status).

## T-12 producer — Docs: threat model, README deploy, Docker artifacts

Produced by: Tech Writer. Date: 2026-08-04. Scope: G-9 (THREAT-MODEL), README deploy/ops guide, `docker/` artifacts. REVIEW ONLY — không sửa code runtime (`loadtest/`, `src/`).

### Delivered

| Artifact | Nội dung | Verification |
|---|---|---|
| `docs/THREAT-MODEL.md` (v0.1 → v0.2) | Audit từng control (TH-1..TH-13) với code FINAL; cập nhật 6 mục stale (TH-1/2/3/4/5/12/13 — control "—" → đã xử lý, ghi file:line thật); thêm **TH-14** (health false-ok — T-07 fix, health.ts); TH-7 cập nhật **browser-WS-auth** (handshake.auth fix 2 phía); TH-11 cập nhật **JSONL sink** (`LOGTEST_LOG_FILE`, rotation 10MB×5); residual: `X-Frame-Options` delegate nginx (docker/nginx.conf), browser-WS-auth cross-repo deploy note | Spot-check ≥ 10 threat controls khớp code: rate-limit 5/60s (config.ts:195-196, api-server.ts recordFail), register gate 403 REGISTER_DISABLED (api-server.ts:219), CORS echo+Vary (http-server.ts), CSP meta build-only + connect-src explicit (vite.config.ts:14-58), gateway auth order (websocket.gateway.ts:147-150), health degraded/down (health.ts:72-85), finalize barrier (server.ts:59-77), redaction (logger.ts:90-135) |
| `README.md` | 7 sections: Cài đặt & chạy (+ bảng 28 env keys loadtest từ config.ts/.env.example), Bảo mật (verify giữ nguyên — khớp .gitignore/gitleaks/pre-commit), **Database** (up/down/status/cleanup + pg_dump + retention), **Vận hành** (health, /metrics, JSONL, graceful shutdown, TTL 12h + banner, limitation 1 coordinator = 1 run), CI & quality (giữ — khớp ci.yml/init-test-dbs.mjs), **Deploy** (Docker build/run + giả định), **Triển khai gateway change** (handshake.auth, repo riêng, thứ tự deploy) | Các lệnh đối chiếu package.json scripts + api-server.ts routes (`/metrics`, `/api/loadtest/health`) |
| `docker/Dockerfile.frontend` | 2-stage node:22-alpine (`npm ci` + `npm run build`, ARG VITE_* có default) → nginx:alpine + `docker/nginx.conf`; healthcheck `wget -qO- http://localhost/healthz` | Review cú pháp — hợp lệ; `npm ci` cần package-lock.json (đã commit) |
| `docker/nginx.conf` | SPA fallback `try_files $uri $uri/ /index.html;` (CRITICAL react-router), `location = /healthz` 200, **KHÔNG CSP header** (D-8 — single source meta build), gzip, cache `/assets/` immutable 1y, index.html no-store, `X-Frame-Options: DENY` + `nosniff` (D-10 header-only delegate) | — |
| `docker/Dockerfile.loadtest` | node:22-alpine; `npm ci` (có devDeps — tsx là devDep, document); copy `package.json` + `loadtest/` (health.ts import `../package.json`); CMD `npm run loadtest:server`; healthcheck `wget -qO- http://localhost:3401/api/loadtest/health` (200 kể cả degraded — D-25); ENV mặc định `LOADTEST_HOST=0.0.0.0`, `LOADTEST_DATA_DIR=/data` | — |
| `docker/.dockerignore` | (thêm — xem deviation) chặn `node_modules`, `.env*`, `loadtest/.env`, `loadtest/data` (secret), `dist/coverage/docs` | — |

### Key facts confirmed
- Env JSONL sink = **`LOGTEST_LOG_FILE`** (prefix lệch so với `LOADTEST_*` — đúng theo logger.ts:220-227 + .env.example:87; README ghi chú tránh gõ nhầm).
- Health route = `/api/loadtest/health` (trong prefix); tool-metrics = `/metrics` (ngoài prefix, public) — không trùng `/api/loadtest/metrics` (tick-history).
- Gateway change (handshake.auth) đã commit ở repo gateway-auth-service — README mục "Triển khai gateway change" ghi thứ tự deploy + hệ quả nếu deploy trễ (browser fallback polling).

### Deviations (nhỏ, có lý do)
1. **`docker/.dockerignore` thêm ngoài 3 file yêu cầu** — không có nó, `COPY . .` trong Dockerfile.frontend sẽ (a) ghi đè node_modules Linux bằng binary Windows (vỡ build), (b) nướng `loadtest/.env` + `loadtest/data/*` (secret thật) vào image — vi phạm "Dockerfile không chứa secret".
2. nginx.conf set `X-Frame-Options`/`nosniff` ngoài yêu cầu tối thiểu — theo đúng THREAT-MODEL residual (D-10: delegate header-only directive riêng cho nginx); **vẫn không set CSP header** (D-8 — ràng buộc cứng).

### Status
**T-12 DONE — sẵn sàng W4 Reality Check** (G-7: 2 Dockerfile build trên test env + THREAT-MODEL G-9). Chưa chạy docker daemon — build thật để Reality Checker xác nhận.

## T-12 review — Security/Docker

Security Architect + Reality Checker. REVIEW ONLY (không sửa code). Đối chiếu THREAT-MODEL v0.2 từng control với code FINAL; README 7 mục vs config.ts/package.json; 3 Dockerfile + .dockerignore. Đã đọc: THREAT-MODEL.md, README.md, docker/nginx.conf, Dockerfile.frontend, Dockerfile.loadtest, .dockerignore, .gitignore, .gitleaks.toml, vite.config.ts, loadtest/{config,rate-limit,http-server,api-server,health,logger,auth,server,guards}.ts, db/{store,writer,cleanup}.ts, routes/settings.ts, src/lib/socket.ts, eslint.config.js, gateway-auth-service websocket.gateway.ts:147-150.

| # | Mục | Severity | Verdict | Chi tiết (đã verify từng dòng) |
|---|---|---|---|---|
| F-1 | TH-2 + Residual: claim "eslint `react/no-danger`" | **Medium** | **DRIFT — claim sai** | Rule KHÔNG tồn tại trong eslint.config.js (grep = 0). Worse: `MatchingScreen.tsx:131` dùng `dangerouslySetInnerHTML={{__html: search}}` — hiện chỉ nhận chuỗi hardcode (DEMO_PEOPLE, `setSearch` 3 nơi) nên **chưa** exploit được, nhưng đây đúng pattern mà rule claimed sẽ chặn; ai đó đổi `p.n` sang member displayName (server-controlled qua gateway) là XSS. Doc khai 1 control không có — cần sửa v0.3 hoặc thêm rule thật |
| F-2 | nginx.conf add_header inheritance | **Medium** | **Config bug — control mất hiệu lực 1 phần** | nginx: add_header ở location **không kế thừa** từ server level. `location = /index.html` + `location /assets/` có `add_header Cache-Control` riêng → `X-Frame-Options: DENY` + `nosniff` **KHÔNG được gửi** cho index.html (chính là document frameable — `/` cũng internal-redirect về `= /index.html`). THREAT-MODEL residual "đã delegate XFO cho nginx" + README "nginx set X-Frame-Options" sai với response HTML chính. Fix: lặp 3 header trong 2 location đó (hoặc dùng map). /healthz + / đúng nhưng /healthz không quan trọng |
| F-3 | TH-5: "KHÔNG bao giờ `*`" | Low | **DRIFT nhẹ** | http-server.ts `parseOrigins` giữ `'*'` + `originAllowed` trả true → `LOADTEST_CORS_ORIGIN=*` **được honor** (ACAO echo mọi origin). Default an toàn, nhưng "KHÔNG bao giờ" là overstate. Doc nên ghi "mặc định allowlist; config rõ ràng `*` vẫn có hiệu lực" |
| F-4 | README bảng env loadtest | Low | **Thiếu 2 key** | config.ts đọc 30 key `LOADTEST_*`; README bảng 30 dòng nhưng thiếu `LOADTEST_REGISTER_RAMP` (config.ts:185) + `LOADTEST_FIXTURE_POST_IDS` (config.ts:186). 28 key kia khớp name+default. Quirk `LOGTEST_LOG_FILE` (logger.ts:220, không phải LOADTEST_) — **chính xác** ✓ |
| F-5 | README §build comment | Low | Nit | Dòng 25 ghi `tsc -b && vite build` nhưng package.json + README:198 = `tsc --noEmit && vite build` — tự mâu thuẫn trong README |
| F-6 | TH-9 regex literal | Low | Nit | Doc `/^lt[a-z0-9]{2,24}$/i`; code `[a-z0-9-]` (http-server.ts:136, `-` cho backward compat) — doc thiếu `-` |
| F-7 | docker/.dockerignore vị trí | Info | Nit | Nằm cạnh Dockerfile (docker/) — chỉ Docker ≥ 23.0 BuildKit honor; chưa ghi yêu cầu version. Hiện tại đúng |
| F-8 | README "tạo 7 bảng" | Info | Nit | 001_init.sql = 6 bảng data + `schema_version` = 7 total — wording gợi ý 7 data bảng + version riêng |

**Xác nhận ĐÚNG (không drift)** — TH-1 (.gitignore `loadtest/data/*` + `!loadtest/data/.gitkeep`, .gitleaks.toml allowlist test-only, secret:scan + pre-commit, data/.gitkeep tồn tại), TH-2 CSP (vite.config.ts injectCspMeta: script-src 'self', connect-src explicit 'self'+2 fonts+gateway+VITE_CSP_CONNECT_SRC, **không** ws: wildcard), TH-3 (rate-limit.ts FailWindow 5/60s/IP; api-server.ts recordFail post-response 4xx=1 fail + 429/Retry-After/X-RateLimit-*; LOADTEST_RATE_LIMIT_DISABLED), TH-4 (parseBool no-default → false; gate trước body validation + rate; 403 REGISTER_DISABLED api-server.ts:218-221; /config.allowRegister settings.ts:30), TH-5 CORS (echo + Vary: Origin, preflight 204, default http://localhost:5173), TH-6 (pools route strip password — metadata only), TH-7 (src/lib/socket.ts:92-93 auth+extraHeaders; socket-farm.ts:100-101; gateway websocket.gateway.ts:147-150 đúng thứ tự auth→query→header — **file:line khớp tuyệt đối**), TH-8 (500 generic + requestId, không err.message), TH-11 (store.ts error {code,message,context} không sql/params:240-243; logger.ts redactSensitiveFields/redactMsg mọi sink; JSONL rotation 10MB×5; .env.example:87), TH-12 (server.ts thứ tự shutdown + timeout; writer.ts finalizePromise barrier), TH-14 (health.ts buildHealth ok/degraded/down + workerAlive + createHealthProbe 10s; Dockerfile.loadtest healthcheck chấp nhận degraded). README: migration/backup/retention khớp package.json scripts; cleanup --older-than + không đụng admin_users (cleanup.ts:64-67); thứ tự deploy gateway đúng (gateway trước/cùng, fallback polling). Docker: SPA fallback ✓, /healthz 200 ✓, KHÔNG CSP header (D-8) ✓, gzip ✓; Dockerfile.frontend 2-stage ✓ ARG VITE_* vào env RUN (build-time đúng) ✓ .env bị .dockerignore chặn ✓; Dockerfile.loadtest npm ci devDeps (tsx) ✓ package.json copy (health.ts import ../package.json) ✓ LOADTEST_DATABASE_URL qua env ✓ migration 1 lần documented ✓; **grep docker/ = 0 secret nướng vào image** ✓; `.env.example` re-include (placeholder, vô hại) ✓. THREAT-MODEL v0.2 vẫn 1 trang (52 dòng) ✓ đọc được.

**Verdict**: KHÔNG blocker. 2 Medium cần sửa trước W4 (F-1 doc drift + rule thật thiếu; F-2 nginx header suppression — chỉ là header, không phải lỗ hổng exploit hiện hữu), còn lại Low/Info. **SAFE TO COMMIT** với điều kiện ghi 2 finding này vào THREAT-MODEL v0.3 hoặc ticket fix nhỏ.

## T-11 review — API/Perf

API Tester + Performance critic. REVIEW ONLY (không sửa code). Verify adversarial: contract test vs dispatcher thật, type 2-chiều (test phá field — fail thật rồi revert), mock gateway vs client thật, chạy 2 suite + mutation thật. Đã đọc: loadtest/__tests__/{contract,types-contract.typecheck,types-contract,mock-gateway,e2e-mock-gateway,coordinator-state,config,metrics,report,health}.test.ts, api-server.ts (ROUTES), src/types/loadtest.ts, loadtest/types.ts, auth-factory.ts, rest-actions.ts, socket-farm.ts, coordinator.ts, tool-metrics.ts, db/{writer,store}.ts, stryker.config.mjs, vitest.mutation.config.ts, package.json, .github/workflows/ci.yml.

| # | Mục | Severity | Verdict | Chi tiết (đã verify thật) |
|---|---|---|---|---|
| F-1 | E2E assert `dbWriteFail = 0` | Low | **VACUOUS — không đo gì thêm** | `toolMetrics.dbWriteFail` CHỈ tăng trong DbStore thật (store.ts:226/241/253). E2E dùng RecordingDbWriter đi thẳng qua coordinator, không chạm store → delta luôn = 0, assert pass vô điều kiện. Bằng chứng thật nằm ở count trực tiếp (writeRunStartCount=1, writeRunFinishCount=1, pushTickCount>5) — các assert này vững. Không phải blocker; có thể bỏ dòng delta hoặc đổi thành "RecordingDbWriter không throw" |
| F-2 | Fake Redis che giấu fail Redis thật | Low | **Chấp nhận — giới hạn đã ghi chú** | FakeRedis chỉ có set/get/ping/connect/quit + TTL-expire — đủ đúng bề mặt coordinator dùng (OTP seed + health; poll queue-count đi REST). Method thiếu → throw (test đỏ, không mask im). Không phủ được lỗi Redis thật (refused/timeout/sai URL) — nhưng đó là phạm vi health.test.ts + real run, không phải mục tiêu E2E. Ghi chú sẵn trong header test |
| F-3 | Contract success-envelope chỉ assert /status + /health | Low | **Chấp nhận** | 24 route protected assert đủ 401-envelope; 200-shape sâu từng route để api-server.test.ts (29 tests) lo — phân chia hợp lý. Đủ để khoá EXTERNAL contract (method + path + envelope) |
| F-4 | report.ts mutation score 73.5% | Info | **Sát ngưỡng — theo dõi** | Thấp nhất 4 module (config 76.96, coordinator-state 78.19, metrics 85.22, report 73.50); survivor điển hình là StringLiteral tầm thường (log message, encoding 'utf8' → '') — tác động thấp. Ngưỡng 70 còn dư ~3.5 điểm; suite mọc thêm code report sẽ cần test bù |
| F-5 | loadtest:test không hermetic | Info | **Chấp nhận — CI đã xử lý** | store/migrate/int test dùng Postgres THẬT (stderr có 23505/23503 thật từ dev DB). CI ubuntu leg chạy container postgres:16, windows skip-if-no-DB — pattern có sẵn (ci.yml:37-43) |

**Xác nhận ĐÚNG (verified thật, không chỉ đọc)**:
- **Contract (28/28 route phủ kín dispatcher)**: 24 protected (401-envelope: success=false, statusCode=401, message≠'', timestamp number, requestId string + header `x-request-id`) + health 200-envelope + /metrics text/plain + register 403 REGISTER_DISABLED (gate mặc định false) + login 503 không-DB + 404 route lạ + 404 sai method + CORS preflight 204/ACAO echo không `*`. Đối chiếu từng dòng ROUTES api-server.ts:70-104 — KHÔNG route sót.
- **Type 2-chiều — thí nghiệm phá drift THẬT**: đổi `LoadTestTick.runId` → `runIdx` trong src/types/loadtest.ts → `npm run loadtest:typecheck` FAIL cả 2 chiều (TS2741 tại types-contract.typecheck.ts:50 và :52) → revert, typecheck xanh. CI chạy `loadtest:typecheck` (ci.yml:107) — gate drift có thật trong pipeline. Runtime half (types-contract.test.ts: ACTION_TYPES ≡ ACTION_LABELS, RUNNING/TERMINAL/ACTIVE) có nghĩa, nhưng là encode lại literal — nửa compile-time mới là chốt chính.
- **Mock gateway khớp 1:1 contract client gọi**: register 3 bước (verify-email → verify-sms-otp → complete với registrationKey/phoneKey/accessToken), login, getAll/detail/view/comments/like, chat match + DELETE + my-room + queue-count, PUT rooms/{id}/my-topic, /metrics Prometheus, socket.io auth-token → matching:found (kèm pending-emit chống race connect) + chat:join→chat:joined + chat:send→echo chat:message khớp clientMsgId. Đối chiếu auth-factory.ts:261-318, rest-actions.ts:37-205, socket-farm.ts:94-170 — không lệch.
- **E2E thật**: 12 users × 30s (durationMin 0.5), chạy 30.7s, assert: phase 'finished', runId /^lt/, report đủ section (usersCreated=12, perAction>0, errors/bottlenecks arrays, durationSec≥20), socketConnections≥12, echoSent/echoOk>0, successTotal>0, DB pipeline đúng thứ tự, 3 file report (json/md/csv) tồn tại. Không Postgres (RecordingDbWriter), không Redis thật (vi.mock ioredis). Runtime ~31s suite — chấp nhận được cho CI (test riêng chạy song song).
- **Mutation chạy THẬT (2m25s)**: 4 module đúng target, `thresholds.break: 70` enforced — **score 77.04% (808 killed / 217 survived / 25 no-cov / 0 errors), exit 0 PASS ngưỡng**. vitest.mutation.config.ts chỉ include 4 test file — KHÔNG kéo E2E/contract vào vòng mutant.
- **Suite xanh**: `npm run loadtest:test` = 301/301 (18 files, 33s); `npm run test` = 417/417 (30 files, 32s).
- **Hygiene 5 file spot-check (coordinator-state/config/metrics/report/health)**: không test vô nghĩa — boundary chính xác (50 vs 50.1, 30 vs 30.1, 2000 vs 2001, secret 32 ký tự), message chuỗi chính xác (diệt string mutant), snapshot deterministic (createdAt cố định), fake timers đúng cách.

**Verdict**: KHÔNG blocker. F-1 (assert vacuous) + F-2 (giới hạn fake) là Low, không ảnh hưởng giá trị E2E — bằng chứng thật (pipeline DB count + echo flow + file report) đứng độc lập. Mutation 77.04% ≥ 70 ngưỡng, CI có cả typecheck 2-chiều lẫn 2 suite. **SAFE TO COMMIT.**

## T-10 review — Code Review

Code Reviewer critic. REVIEW ONLY (không sửa code). Adversarial verify THẬT: parse YAML ci.yml, scan gitleaks THẬT bằng binary v8.30.1 (docker `zricethezav/gitleaks`, working-tree + config repo), chạy THẬT postgres:16 container + `init-test-dbs.mjs` + toàn bộ loadtest suite (tái hiện ubuntu leg), 4 gate npm + workspace test + coverage. Đã đọc: ci.yml, .gitleaks.toml, eslint.config.js, package.json, vitest.config.ts, vitest.workspace.ts, loadtest/vitest.config.ts, .github/ci/init-test-dbs.mjs, loadtest/__tests__/{store,migrate,e2e-mock-gateway}.test.ts, loadtest/db/store.ts.

| # | Mục | Severity | Verdict | Chi tiết (đã verify thật) |
|---|---|---|---|---|
| F-1 | `.gitleaks.toml` allowlist key `strings` INERT trên gitleaks hiện đại | **HIGH — CI gitleaks step sẽ RED ngay lần push đầu** | CONFIRMED | gitleaks đổi `strings` → `stopwords` (verify v8.30.1: config chỉ có `strings` → error `"[[allowlists]] must contain at least one check for: commits, paths, regexes, or stopwords"`; config có cả regexes+strings → load OK nhưng `strings` BỊ BỎ QUA im lặng). Scan thật với config repo: `auth.test.ts:9` (`test-secret-0123456789abcdef`) VẪN flag dù allowlist ghi "test-secret" — chính vì `strings` vô hiệu. gitleaks-action@v2 tự tải binary LATEST → CI lần push đầu FAIL với 3 findings: `auth.test.ts:9`, `e2e-mock-gateway.test.ts:106` (`e2e-otp-secret-…`), `logger.test.ts:92` (JWT header `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9`). Fix đã kiểm chứng: `strings` → `stopwords` (minimal file: hết flag) NHƯNG vẫn còn 2 findings (e2e + logger) — cần allowlist thêm `e2e-otp-secret`/`e2e-auth-secret` + literal JWT header (scan thật với config sửa xác nhận còn đúng 2) |
| F-2 | `loadtest/.env` chứa secret THẬT + gate `npm run secret:scan` đỏ ngay bây giờ | HIGH (gate) / MED | CONFIRMED | `.env:18,20` = LOADTEST_AUTH_SECRET/LOADTEST_OTP_SECRET 64-hex THẬT. Được `.gitignore` chặn (`git check-ignore` → pattern `.env`, không tracked) nên KHÔNG vào history → action CI không thấy. Nhưng `secret:scan`/`secret:scan:tree` (chế độ `--no-git` working tree) FAIL 2 findings ngay hiện tại — docs T-03 ghi "PASS" (line 104) là STALE (khả năng verify bằng gitleaks cũ khi `strings` còn hiệu lực). Fix: thêm allowlist regex cho loadtest/.env (hoặc xoá/rotate secret dev) |
| F-3 | `loadtest:test` (ci.yml:114) + `test` workspace (ci.yml:119) chạy DB suite 2 lần | LOW | Chấp nhận | Workspace test chạy LẠI toàn bộ loadtest suite gồm store/migrate integration + E2E 30s — ~30s phí, không fail (TRUNCATE/DROP idempotent trên DB tách biệt). Có thể bỏ step `loadtest:test` hoặc dùng `--project` filter |
| F-4 | eslint ignores thiếu `.stryker-tmp/**` | LOW | CONFIRMED | Sau khi stryker chạy, `eslint .` ra 7 warning `.stryker-tmp/sandbox-*/…` (0 errors — exit 0). Thêm vào ignores + .gitignore |
| F-5 | `.github/ci/init-test-dbs.mjs` ngoài coverage eslint | INFO | Nit | Không khớp files pattern nào trong flat config → không được lint (mjs chứa console.log — vô hại, script nhỏ). Ghi chú thôi |
| F-6 | Windows leg gitleaks chưa có bằng chứng chạy thật | INFO | Hedged đúng cách | Workflow tự ghi chú rủi ro + hành động ghi nhận README. Action tải binary Windows hợp lệ (gitleaks publish windows build) — chấp nhận được với hedge sẵn có |

**Xác nhận ĐÚNG (verified thật, không chỉ đọc)**:
- **ci.yml YAML hợp lệ** (Ruby psych parse OK). Matrix windows: Postgres dùng `docker run` + guard `if: runner.os == 'Linux'` + `shell: bash` — skip ĐÚNG trên Windows; mọi step dùng chung là npm scripts hoặc `node .mjs` — không bash-only syntax. Claim "services: hard-fail windows" đúng (service container chỉ Linux runner — GitHub docs) — cách `docker run` là hợp lệ.
- **DB leg ubuntu chạy THẬT**: postgres:16 container (dùng port 54321 vì 5432 máy dev đã bị chiếm — logic y hệt CI), `init-test-dbs.mjs` tạo đúng `loadtest_test` + `loadtest_test_migrate`; env `LOADTEST_TEST_DATABASE_URL`/`LOADTEST_TEST_MIGRATE_DATABASE_URL` khớp TÊN ENV mà store.test.ts:18 + migrate.test.ts:14-15 đọc, port 5432 khớp mapping container. `npm run loadtest:test` = **301/301 (18 files)** — store (15) + migrate (14) integration chạy THẬT trên container, xanh.
- **Windows leg an toàn**: store/migrate probe 3s (connectionTimeoutMillis: 3000) → ECONNREFUSED localhost → `describe.skip`, không hang; e2e dùng RecordingDbWriter (KHÔNG cần Postgres — không có nguy cơ `dbWriteFail` khi DB tắt).
- **setup-node cache: npm** → key tự động theo hash package-lock.json ✓. **upload-artifact@v4** `if-no-files-found: ignore` + `if: always()` ✓, tên `coverage-${{ matrix.os }}` hợp lệ. **Coverage**: chạy sau test, `--config vitest.config.ts` frontend-only, thresholds 70% — thực tế **99.47% stmts / 88.4% branch / 100% funcs** ✓.
- **gitleaks-action@v2**: action.yml KHÔNG khai inputs (fetch xác nhận claim comment); `GITLEAKS_LICENSE` chỉ bắt buộc cho Organization (README xác nhận); hard-fail trên leak nhất quán docs action (không phản chứng được). Config `.gitleaks.toml` auto-detect từ source dir (verify: scan không `--config` = scan có `--config` → 5 findings như nhau) → action sẽ dùng config repo — nhưng chính vì F-1 config vô hiệu.
- **4 gate npm xanh**: `npm run lint` 0 errors (7 warnings transient F-4), `typecheck` ✓, `loadtest:typecheck` ✓, `build` ✓. `npm run test` = **417/417 (30 files)**. ESLint flat config sạch (không deprecated style): src strict (`no-console` error + 4 override có lý do ghi chú, `react-refresh` allowConstantExport), loadtest `no-console` off đúng phạm vi (logger.ts/CLI là interface), node globals cho root configs; eslint/plugin toàn devDependencies — không runtime pollution (loadtest zero-dep giữ nguyên).

**Verdict**: **KHÔNG SAFE TO COMMIT ngay** — F-1 làm CI đỏ ngay lần push đầu (gitleaks step fail 3 findings) và F-2 làm gate local `secret:scan` đỏ. Fix nhỏ trong phạm vi T-10: đổi `strings` → `stopwords` + allowlist thêm `e2e-otp-secret`/`e2e-auth-secret` + literal JWT header (đã kiểm chứng bằng scan thật), xử lý `loadtest/.env`. Sau fix → **SAFE TO COMMIT**.

## W4 Reality Check

**Reality Checker — W4 (T-10/T-11/T-12). Default FAIL. Ran actual commands on 2026-08-05; claimed previous-state numbers (417/417, 77.04%) are NOT reproducible on this machine.**

| # | Wave-4 claim | Evidence (ran/read) | Result |
|---|---|---|---|
| 1 | `npm run test` 417 green | `npm run test` → **12 failed suites** (all frontend), 261 passed + 40 skipped (301) = loadtest-only. Frontend fails at COLLECTION: `Error: Vitest failed to find the current suite` thrown by `afterEach(() => cleanup())` at `src/test/setup.ts:9`. Reproduced standalone (`npx vitest run src/lib/env.test.ts`), with `--pool=forks`, and with a bare no-import test in `src/` — all fail identically. The workspace merges loadtest+frontend; frontend tests never collect → 417 is unattainable now | **RED** |
| 2 | CI `ci.yml` valid + matrix + Postgres + gates | YAML valid (Ruby psych). Matrix ubuntu+windows ✓, gitleaks-action@v2 step ✓, Postgres16 container `if: runner.os == 'Linux'` ✓, lint/typecheck/loadtest:typecheck/build/loadtest:test/test/coverage + upload-artifact ✓. Env `LOADTEST_TEST_DATABASE_URL`/`LOADTEST_TEST_MIGRATE_DATABASE_URL` match store.test.ts:18 + migrate.test.ts:15 ✓ — but `LOADTEST_TEST_API_DATABASE_URL` (api-server.test.ts:19) is NOT set and `init-test-dbs.mjs` does not create `loadtest_test_api` → api-server DB suite will skip on ubuntu CI (minor gap) | **PARTIAL** |
| 3 | gitleaks config (FIX-1/FIX-2) | `.gitleaks.toml` uses `stopwords` key (not `strings`) ✓ with `test-secret`/`e2e-otp-secret`/`e2e-auth-secret` ✓ + JWT fixture regex ✓ + `paths` for `.env` ✓. But gitleaks binary NOT installed → `npm run secret:scan` exits "not recognized" | **UNVERIFIED** (config correct on read; scan needs `winget install gitleaks`) |
| 4 | Mutation ≥70 (G-2) | `stryker.config.mjs` targets coordinator-state/metrics/config/report, `thresholds.break: 70` ✓. **`npm run loadtest:mutation` FAILS**: `ConfigError: No tests were executed. Stryker will exit prematurely.` exit **1**. Root cause: stryker vitest-runner `vitest.related` mode "failed to find test files related to mutated files" (all 4 test files DO import their sources directly). Last-known 77.04% NOT reproducible | **RED** |
| 5 | Contract wired (G-3) | contract.test.ts = 31 tests ✓ runs in loadtest:test. types-contract.typecheck.ts exists + included via `loadtest/tsconfig.json` (`__tests__/**/*.ts`) → wired into `loadtest:typecheck` (passes). Bidirectional structural assignments; drift-fail was verified experimentally in prior review (ts typecheck) | **PASS** |
| 6 | E2E (G-7) | e2e-mock-gateway.test.ts exists, ran in loadtest:test in 30.3s, 1 test, 12 users × 30s (targetUsers:12, durationMin:0.5) ✓; mock-gateway.ts present | **PASS** |
| 7 | THREAT-MODEL (G-9) | One page, 14 threats (TH-1..TH-14) ≥13 ✓, each with a control. Spot-checked 7 file refs resolve: `vite.config.ts:21` injectCspMeta ✓, `http-server.ts:59` applyCors ✓, `health.ts:110` createHealthProbe ✓, `api-server.ts:68` ROUTES ✓, `auth.ts:15` SESSION_TTL_MS=12h ✓, gateway `websocket.gateway.ts:147-150` (handshake.auth?.token) ✓, rate-limit.ts/logger.ts exist ✓. One line drift: `config.ts:91` cited for LOADTEST_HOST, actual 169. CSP meta verified in built `dist/index.html` (explicit connect-src, no ws:/wss: wildcard) | **PASS** (minor line-drift) |
| 8 | Docker sane (FIX-4) | Dockerfile.frontend + Dockerfile.loadtest + nginx.conf + .dockerignore exist ✓. nginx: SPA fallback `try_files $uri $uri/ /index.html;` ✓, NO CSP add_header (meta-only, documented) ✓, security headers on `/index.html` + `/assets/` + `/healthz` ✓. Secret grep in docker/ → only legit ARG names, no values ✓ | **PASS** |
| 9 | README 7 sections + env table | README has **14** H2 sections (not 7). Env table: 32 loadtest keys + 4 VITE keys; all rate-limit/worker/max keys present ✓. Gaps: `LOADTEST_ADMIN_EMAIL/USERNAME/PASSWORD`, `LOADTEST_SEED_ADMIN` (db/init.ts), `LOADTEST_PROVISION_CONCURRENCY` (auth-factory.ts), `LOADTEST_WORKER_ID` (worker.ts) NOT documented; `loadtest/.env.example` also omits `LOADTEST_AUTH_SECRET`, `LOADTEST_FIXTURE_POST_IDS`, `LOADTEST_WORKERS` | **PARTIAL** |
| 10 | Cleanliness | `git status --short` = only expected paths (loadtest/, src/, docs/, docker/, .github/, config files). `loadtest/.env` gitignored ✓. No committed .env/data ✓. (My mutation run left `.stryker-tmp/` — untracked + NOT in .gitignore; removed after verification — repo hygiene gap, add to .gitignore) | **PASS** (1 hygiene nit) |

**Verdict: FAIL — NOT SAFE TO COMMIT.** Two hard blockers: (1) `npm run test` frontend suite is RED (12 files fail at collection in `src/test/setup.ts:9` — the workspace build broke the `afterEach` hook registration on this machine; CI runs this step, so CI would be red on the Windows leg and is unverifiable on ubuntu); (2) `npm run loadtest:mutation` exits 1 (Stryker `vitest.related` detection failure) — the G-2 mutation ≥70 gate cannot be executed, so the 77.04% claim is unproven. Open: gitleaks binary not installed (config correct on read, scan unverified); CI cannot be executed without a GitHub remote; `LOADTEST_TEST_API_DATABASE_URL` missing from CI env; README env table + section-count drift.

## Phase 4 — Perf/Reliability council

Performance Benchmarker + SRE. REVIEW ONLY (không sửa code). Default HOÀI NGHI. Lens: WHOLE-SYSTEM PERFORMANCE + RELIABILITY ở 5k users. Đã đọc toàn bộ hot path (coordinator.ts, db/writer.ts, db/store.ts, socket-farm.ts, worker-farm.ts, logger.ts, util.ts, rest-actions.ts, http.ts, metrics.ts, rate-limit.ts, coordinator-state.ts, health.ts, api-server.ts, server.ts, config.ts, db/int.ts, db/cleanup.ts) + ci.yml + package.json + stryker/vitest configs; chạy THẬT `npm run loadtest:test` = **301/301 (18 files, 261 passed + 40 skipped-no-DB, 31.5s — E2E 30.3s) exit 0**. Đối chiếu gateway-side dedupe: gateway-auth-service `chat-socket.service.ts:158-160` (clientMsgId → eventId Kafka) + content-service `chat-message.service.ts:482-499` (key namespace theo userId).

| # | Mục | Severity | Verdict | Chi tiết (đã verify thật) |
|---|---|---|---|---|
| F-1 | Vòng lặp log tự duy trì khi DB chết GIỮA run | **MAJOR** | CONFIRMED | DB down trong lúc run (đúng kịch bản "best-effort" thiết kế hứa) → mỗi `insertLogEvent` fail (store.ts:242 `ltLog.warn`) → subscriber logger (writer.ts:53 `subscribeLog` → writeLog) → `insertLogEvent` fail mới → warn mới → ... vòng lặp bất tận, tốc độ = latency fail của 1 query (ECONNREFUSED không nằm trong TRANSIENT_CODES int.ts:36 → fail nhanh, không sleep retry). Hệ quả: spam console, churn ring buffer 500, `dbWriteFail` inflate, JSONL sink bị `appendFileSync` sync mỗi dòng nếu bật. KHÔNG giết run nhưng làm coordinator thoái hoá; tự dừng khi DB hồi phục. Không có reentrancy guard. Fix đề xuất: không re-log lỗi DB từ trong subscriber, hoặc debounce/throttle writeLog |
| F-2 | Claim "5xx/timeout retry 1×" — timeout KHÔNG được retry | MEDIUM | CONFIRMED | rest-actions.ts:93 `if (res.ok || res.failClass !== 'SERVER') return res;` — timeout/network → `failClass 'NETWORK'` (http.ts:118) → KHÔNG retry. Header comment (rest-actions.ts:6 "5xx/timeout") sai với code: chỉ 5xx (SERVER) retry 1×, 4xx không retry (đúng). Timeout là lỗi transient ĐÁNG retry nhất — doc-vs-code drift có thật |
| F-3 | log_events + metric_samples tăng không giới hạn | MEDIUM (ops) | CHẤP NHẬN — flag ops | Mỗi log line lúc run = 1 INSERT riêng (không batch); 1h run ≈ 3600 metric_samples + vài trăm log_events. Retention CHỈ thủ công (`loadtest/db/cleanup.ts --older-than`, không scheduler, không cron trong docker/). Không index trên runs.start_at cho DELETE retention. Đã ghi nhận là known ops issue — cần cron/task nhắc |
| F-4 | DB write KHÔNG có statement timeout | LOW | CONFIRMED | Chỉ probe có query_timeout 2s (store.ts:196); flushTicks/finalizeRun/insertLogEvent không timeout → DB treo (không phải refuse) → flush chờ vô hạn, shutdown deadline 10s → force exit 1, finalize có thể bị drop; `dbWriteFail` KHÔNG tăng khi hang (khác fail) → hang vô hình ngoài health probe |
| F-5 | Worker tick vẫn gọi `process.memoryUsage()` mỗi 1s | LOW | CONFIRMED (nhỏ) | Claim "rssMb moved to 5s" CHỈ đúng cho coordinator gauge (coordinator.ts:284, metricsTimer 5s riêng). Worker emitTick (socket-farm.ts:663) vẫn tính rssMb mỗi giây + ship qua IPC — coordinator KHÔNG dùng field này (agg.tick.workers chỉ cpuAvg). Chi phí ~µs — vô hại, nhưng là field chết + claim quá rộng |
| F-6 | `shutdownTimeoutMs` env-overridable không có floor | LOW | CONFIRMED | server.ts:63-67 deadline = env.shutdownTimeoutMs (config.ts:200 default 10_000). "Tổng timeout ≥ 10s" chỉ đúng ở default — set `LOADTEST_SHUTDOWN_TIMEOUT_MS=2000` vẫn pass validate → tổng timeout < 10s. Nên `Math.max(10_000, ...)` |
| F-7 | Histogram cumulative rebuild mỗi tick 1s + actionsPerSecSeries không cap | INFO | Chấp nhận | coordinator.ts:423-428 rebuild `new ActionHistograms()` + merge O(W×A×48) mỗi giây — bounded (10 worker × ~10 action = ~5k op/s + GC churn nhỏ, chấp nhận). `actionsPerSecSeries` (coordinator.ts:441) push 1/s KHÔNG cap (tickHistory cap 3600) — 86k entry/ngày run dài; tầm thường nhưng không bounded |
| F-8 | Mutation KHÔNG có trong CI | INFO | Chấp nhận — đề xuất nightly | ci.yml không có step mutation (G-2 là gate local; W4 xác nhận mutation đang FAIL trên máy này). CI wall-time ước lượng: ubuntu leg ≈ 9-12 min (npm ci ~1-2m + gitleaks ~30s + postgres pull/start ~1m + lint/typecheck/build ~2m + loadtest:test 35s + test workspace ~1.5m + coverage ~1m), windows leg ≈ 7-9 min — xa dưới 6h, KHÔNG vượt 15 min/leg. Nếu thêm mutation vào CI: +2.5-3m/leg → đề xuất job nightly ubuntu-only (matrix bỏ windows) thay vì chạy mỗi push |

**Xác nhận ĐÚNG (verified thật, không chỉ đọc)**:
- **Hot path sạch per-message**: socket-farm handler (chat:message/matching:found/chat:error) KHÔNG log; sendChat/echo/outbox không log; coordinator tick 1s không log (chỉ phase transition + E1/E2/E3); `onWorkerMessage 'tick'` không log. `redactMsg` (URL parse try/catch mỗi dòng) chỉ đánh vào lifecycle logs (~1/5s snapshot + transitions) — không đáng kể.
- **Tick 1s bounded**: O(workers) aggregate (coordinator-state.ts:104-144) + O(W×A×48) histogram rebuild; scheduler 100ms chung (không setInterval/user) — 5k users / 10 workers = 500 user-check/100ms/worker, O(1)/user. Snapshot 5s là interval RIÊNG (metricsTimer, coordinator.ts:267) — rssMb coordinator chỉ ở đó (FIX-6 giữ).
- **JSONL sink**: chỉ gọi từ `log()` (logger.ts:266-272) — lifecycle only, không trên hot path; rotation 10MB × 5 file (createJsonlSink). Worker KHÔNG kế thừa sink: `workerEnv` (worker-farm.ts:23-25) ghi đè `LOGTEST_LOG_FILE: ''` → logger.ts:227 falsy → no sink. VERIFIED.
- **DB volume sane**: tick ghi **1 metric_samples row/giây/run** (aggregated — KHÔNG theo user); flush 30s ≈ 30 rows, cap 500 tick (writer.ts:20-21,128) → 5k users chỉ ~3600 rows/h; batch multi-row INSERT 1 lệnh (store.ts:379-403). W2 claim "1 row/s per RUN" giữ.
- **dbWriteFail visibility đủ**: store.query `write:true` (store.ts:226/241/253) → `/metrics` `lt_dbWriteFail_total` (không auth, api-server.ts:71-75) + snapshot log 5s (coordinator.ts:289-296) + health probe db:'down' → degraded/down (health.ts:82-85) + 10s cache probe. Trace writer fail → counter → metrics → health đầy đủ.
- **Memory bounded**: histogram 48 bucket × action (metrics.ts:9), ring buffer 500, outbox cap 1000/user (config.ts:180 `maxPendingOutbox`), pendingTicks cap 1000 drop-oldest (writer.ts:140-147), rate-limit sweep >2048 entry idle 10min (rate-limit.ts:172-175), Redis connect commandTimeout 2s (auth-factory.ts:384). KHÔNG structure unbounded mới (ngoài F-7).
- **Retry/idempotency**: 4xx không retry, 5xx retry 1× backoff 200ms×attempt (rest-actions.ts:83-97) — giữ (trừ F-2 timeout). Reconnect: KHÔNG có resend → không có race double-send phía client; outbox TTL 60s → NO_ECHO (đo lường sai âm khi echo mất lúc disconnect, không phải trùng lặp); SERVER dedupe vẫn enforced: clientMsgId → eventId Kafka idempotent (gateway) + key namespace per-user (content-service). E1/E2/E3 intact: E1 register fail >50% & ≥10 (coordinator.ts:226-230), E2 connect fail >30% & ≥10 (coordinator.ts:463-481), E3 >50% worker chết/60s (coordinator.ts:482-486); allowlist hard-block ở cả coordinator.start (coordinator.ts:152-154) lẫn validateRunRequest (config.ts:278-282).
- **Startup/shutdown**: fail-fast validateEnv → connect (3s timeout, dbRequired throw → exit 1 ~3s khi DB down) → baseline migrations idempotent → import pool → initRedis (catch → redis=null, không block listen) → listen. Shutdown: closeConnections → stop(true) → finishRun AWAIT writeRunFinish (B-2 finalize barrier) → dbWriter.shutdown (await finalizePromise + flush + pool.end) → deadline exit(1) 10s default (F-6). Test thật: `loadtest:test` 301 green 31.5s (E2E 30.3s đúng spec 12 users × 30s).

**Verdict**: KHÔNG Critical. 1 Major (F-1 — vòng lặp log khi DB outage giữa run, vi phạm nhẹ "best-effort không thoái hoá") + 1 Medium (F-2 doc-vs-code retry timeout) + 1 Medium ops (F-3 retention thủ công, đã ghi nhận). Phần còn lại Low/Info. Hot path, DB pipeline, dedupe, auto-stop, health chain đều giữ đúng claim. F-1 nên fix trước merge (guard nhỏ), F-2 sửa comment hoặc code. **SAFE TO COMMIT với F-1/F-2 ghi nhận làm follow-up.**

## Phase 4 — Security council

Security Architect. REVIEW ONLY (không sửa code). Default HOÀI NGHI. Lens: WHOLE-SYSTEM SECURITY — cross-module attack surface, không theo task. Đã verify thật: git history toàn bộ `--all` (tree scan từng commit), `.gitleaks.toml`, cả 3 `.env.example` + `.dockerignore`, route table (api-server.ts:68-105), guards + rate-limit (guards.ts, rate-limit.ts), CORS path (http-server.ts:29-68), logger/JSONL sink + redaction (logger.ts), DB credential path (db/init.ts, db/store.ts, db/migrate.ts), token flows (src/lib/socket.ts, storage.ts, loadtest-auth-storage.ts), CSP build (vite.config.ts), Dockerfile.loadtest + nginx.conf, gateway diff (websocket.gateway.ts:148-150), cả 4 service sibling (content/gateway/user-community: chỉ có `.env.example` tracked). Chạy THẬT `npm run test` = **417 tests (30 files, 377 passed + 40 skipped) exit 0, 31.4s** — đúng expectation.

| # | Mục | Severity | Verdict | Chi tiết (đã verify thật) |
|---|---|---|---|---|
| SEC-1 | CORS `*` bypass được qua env — doc "KHÔNG `*`" (SEC-2) KHÔNG được enforce | **MEDIUM** | CONFIRMED | `parseOrigins` (http-server.ts:36) CHỦ ĐÍCH giữ `*` làm wildcard; `originAllowed` (http-server.ts:48) `if (origins.includes('*')) return true` → operator set `LOADTEST_CORS_ORIGIN=*` = echo MỌI origin thật. `validateEnv` (config.ts:220-261) KHÔNG reject `*`. Contract test chỉ assert default config (contract.test.ts:188), không test `*`. Đúng misconfiguration-dependence; API dùng Bearer (không cookie → không CSRF), nên impact = cross-origin probing + doc-vs-code breach. Fix: reject `*` trong parseOrigins hoặc validateEnv |
| SEC-2 | gitleaks `paths` allowlist phủ MỌI `.env` ở mọi depth | MINOR | CONFIRMED | `.gitleaks.toml:58` `(^|[/\\])\.env$` — nếu ai `git add -f` 1 file `.env` thật, gitleaks sẽ KHÔNG flag (allowlist toàn bộ file, không phải chỉ gitignored local). Defense-in-depth yếu: nên scope theo path cụ thể hoặc bỏ `paths` (dựa vào pre-commit hook). Lịch sử commit hiện tại vẫn sạch — đây là lỗ hổng CHẶN SAU này |
| SEC-3 | Pre-commit hook INERT trên máy này (gitleaks chưa cài) | MINOR | CONFIRMED | Hook (scripts/pre-commit → .git/hooks/pre-commit) `exit 0` khi gitleaks không có; máy hiện tại KHÔNG có gitleaks binary (`gitleaks version` fail) → hook đang không chặn gì. Đúng thiết kế (không block commit hợp lệ) nhưng gate secret đang ngủ. Nên cài gitleaks hoặc CI làm nguồn chặn thật |
| SEC-4 | `redactMsg` regex hở "Bearer <jwt>" (latent) | MINOR (latent) | CONFIRMED | logger.ts:131 `([^\s,;|]+)` sau `authorization:` bắt được "Bearer", để lộ JWT đằng sau nếu 1 msg dạng `authorization: Bearer eyJ...` được log. Hiện KHÔNG có log statement nào emit Authorization header (verify: guards.ts:16 là nơi duy nhất đọc; api-server.ts:255 log method+path+message thôi) → latent. Fix 1 dòng: thêm `(?:Bearer\s+)?` vào pattern |
| SEC-5 | `/metrics` + `/health` public, KHÔNG rate-limit — Docker expose 0.0.0.0 | LOW | Chấp nhận — flag Docker | Route table (api-server.ts:70-75): 4 route public = health, metrics, login (rate 5/60s), register (gate OFF default + rate). `/metrics` chỉ counter/gauge (dbWriteFail, apiErrors, workerRestarts, rssMb, worker.alive) — KHÔNG token/URL → informational. `/health` probe DB cache 10s (health.ts:110) → hammer rẻ, tiết lộ version/uptime/phase = recon nhẹ. `LOADTEST_HOST=0.0.0.0` mặc định trong Dockerfile.loadtest → LAN reachable. Acceptable cho tool nội bộ; KHÔNG deploy tool này ra public internet |
| SEC-6 | Dockerfile.loadtest `npm ci` cài devDeps vào image runtime | LOW | Chấp nhận | tsx (TS launcher) là devDep → image chứa eslint/stryker/vitest/jsdom/@testing-library = bloat + bề mặt supply-chain lớn hơn, nhưng KHÔNG secret (verify .dockerignore: `.env`, `.env.*`, `loadtest/.env`, `loadtest/data` bị loại; package-lock.json committed → deterministic). Cleaner: chuyển tsx sang dependencies hoặc `node --experimental-strip-types` (Node 22) |
| SEC-7 | Gateway change `handshake.auth` — additive, KHÔNG làm yếu gateway | PASS | Verified | websocket.gateway.ts:148-150: `handshake.auth?.token` thêm làm nguồn trích token ĐẦU TIÊN (trước query/header); cả 3 nguồn đi vào CÙNG đường verify: `if (!token) disconnect` → `verifyToken(ACCESS_TOKEN_SECRET)` → `if (!user) disconnect` (websocket.gateway.ts:153-162). Auth field KHÔNG bypass validation. Precedence auth > query > header đúng với frontend (gửi cả 2). Ngoài diff: fallback `ACCESS_TOKEN_SECRET \|\| 'secret'` tồn tại sẵn (pre-existing, không thuộc refactor) |

**Xác nhận ĐÚNG (verified thật)**:
- **Secrets hygiene end-state**: commit order verify — `8c41ad8` "first commit" CHỈ chứa README.md (69 dòng, không secret); W0 (370f2f3) là commit THỨ 2, xoá secrets khỏi working tree. Tree-scan toàn bộ `--all`: KHÔNG commit nào từng track `loadtest/.env`, `auth-secret.json`, `otp-secret.json`, `db-password.json`, hay bất kỳ file nào trong `loadtest/data/` ngoài `.gitkeep`. Cả 3 `.env.example` đều placeholder (root: VITE vars; loadtest: OTP rỗng + `postgresql://USER:PASS@HOST:PORT/DB` + auth secret commented); sibling services chỉ track `.env.example`. `loadtest/.env` hiện diện local + gitignored ✓. stopwords gitleaks chỉ trùng test fixture ("test-secret", "e2e-otp-secret", "e2e-auth-secret", regex anchor dòng logger.test.ts:92) ✓.
- **Token flows end-to-end**: chat — access + refresh token trong localStorage (storage.ts:77-88), socket dùng `auth: {token}` + `Authorization: Bearer` header, CHỦ ĐÍCH không đưa vào query string (socket.ts:88-92) → không rò qua proxy/access log; CSP build-time (vite.config.ts:15-58: `script-src 'self'`, connect-src explicit origins, KHÔNG scheme-wildcard ws:) giảm thiểu XSS-theft. Loadtest — HMAC session token (timingSafeEqual, TTL 12h, auth.ts:33-67) localStorage + Bearer. KHÔNG có console/log statement nào emit token (grep toàn bộ src/ + loadtest/ ngoài logger.redact); JSONL sink nhận entry đã redact (logger.ts:229-246); api error chỉ log method+path+requestId (api-server.ts:255); `/metrics` không label token; log_events nhận msg pre-redacted qua subscriber (logger.ts:259-264).
- **DB data sensitivity**: pool_accounts.password plaintext = accepted + documented. DB credential chỉ trong `loadtest/.env` (gitignored) + env runtime. `db/init.ts:112` log `redactUrl(connectionString)`; admin password KHÔNG BAO GIỜ log plaintext (init.ts:132-135 explicit); migrate.ts/cleanup.ts chỉ log message + đếm. Không có write path nào log password.
- **Attack surface enumeration**: route table đầy đủ (api-server.ts:68-105) — 21 route, 4 public (health/metrics/login/register), 17 auth:true; unknown path → 404; OPTIONS → 204 global. Không còn endpoint unauthenticated nào khác. Rate-limit: login/register 5 fails/60s window/IP, start 1/10s, write OFF default; `LOADTEST_RATE_LIMIT_DISABLED=1` escape hatch chỉ test/CI (documented).

**Verdict**: KHÔNG Critical, KHÔNG Major. 1 Medium (SEC-1 — CORS `*` có thể bật qua env, vi phạm doc SEC-2; fix nhỏ ở parseOrigins/validateEnv) + 2 Minor actionable (SEC-2 gitleaks paths, SEC-4 redactMsg Bearer) + 1 Minor ops (SEC-3 hook ngủ) + 2 Low chấp nhận (SEC-5, SEC-6). Secrets hygiene, token flows, DB path, gateway change đều GIỮ đúng claim. **SAFE TO COMMIT với SEC-1 ghi nhận fix follow-up (1 dòng), SEC-2/SEC-4 đề xuất sửa trước hoặc sau merge.**

## Phase 4 — Correctness council

Code Reviewer (lens: CROSS-MODULE CORRECTNESS — races, idempotency, inconsistent state, contract drift). REVIEW ONLY. Default HOÀI NGHI. Đã đọc toàn bộ chuỗi module: coordinator.ts, coordinator-state.ts, worker-farm.ts, worker.ts, socket-farm.ts, auth-factory.ts, db/writer.ts, db/store.ts, db/schema.sql, logger.ts, tool-metrics.ts, config.ts, api-server.ts, routes/*, api-mappers.ts, health.ts, cleanup.ts, server.ts + frontend (src/types/loadtest.ts, loadtest.store.ts, loadtest-api.ts, app-shell.tsx, ControlPanelPage.tsx, api.ts, auth.store.ts, socket.ts, env.ts) + gateway-auth-service (websocket.gateway.ts:144-166, refreshToken.dto.ts, login.dto.ts, deviceInfo.dto.ts). Chạy THẬT: `npm run loadtest:test` = **301 (18 files, 261 passed + 40 skipped, E2E 30.4s) exit 0**; `npm run test` = **417 (30 files, 377 passed + 40 skipped) exit 0**; `npm run typecheck` + `npm run build` = **PASS**.

| # | Mục | Severity | Verdict | Chi tiết (đã verify thật) |
|---|---|---|---|---|
| C-1 | `start()` lúc run cũ đang COOLDOWN/REPORT — worker orphan + restart config cũ + clobber phase | **MAJOR** | CONFIRMED | `isRunning` (coordinator.ts:99-101) chỉ gồm provisioning/ramping/steady — COOLDOWN/REPORT KHÔNG trong đó → `start()` (coordinator.ts:148-149) không 409 khi run trước đang kết thúc; frontend mirror lỗ (ControlPanelPage.tsx:107 — nút Start ENABLED lúc cooldown). Click start trong ~1-2s đầu cooldown (worker cũ exit sau ~500ms, worker.ts:29-32): (1) `spawnAll` (coordinator.ts:241) GHI ĐÈ handle cũ trong `workers` map (worker-farm.ts:85) không kill child cũ → ORPHAN chạy tải cũ tiếp, `killAll()` run sau (chỉ duyệt map) không bao giờ chạm → leak process + tải giả lên gateway vô hạn; (2) child cũ exit → `onWorkerDied` (coordinator.ts:83-94) thấy isRunning=true → `restart()` (worker-farm.ts:162-176) dùng `this.runConfig` = config run CŨ (chưa setRunConfig mới, không bao giờ null) + `old.accounts` cũ → worker chạy config/accounts cũ; (3) restart trong provisioning lúc farm.total=0 → E3 guard (coordinator.ts:483) trượt; (4) click trúng cửa sổ 'report' (~20-100ms) → `this.phase = phase` (coordinator.ts:577) CLOBBER phase 'provisioning' run mới thành terminal → `transition(...,'ramping')` (coordinator.ts:242) THROW → run mới chết 'error' + DB row kẹt 'running' (finalize UPDATE chạy trước INSERT run mới → 0 row). Fix: start() trả 409 khi phase ∈ {cooldown, report} hoặc `finishing`; frontend disable Start khi phase='cooldown'; spawn ghi đè phải kill child cũ |
| C-2 | Run kẹt 'running' vô hạn khi worker treo — `checkHeartbeats` DEAD CODE + workerTicks không prune + E3 guard `total>0` | **MAJOR** | CONFIRMED | `checkHeartbeats` (worker-farm.ts:147-158) KHÔNG được gọi ở đâu (grep toàn repo: chỉ definition). Worker treo (event loop block, không exit) → không 'exit' event → không restart; `farm.alive` vẫn đếm alive (health 'running'); `workerTicks` (coordinator.ts:343-345) giữ tick cuối vĩnh viễn (không xoá khi worker chết) → aggregation cộng dồn data stale mỗi giây. Treo lúc RAMPING (chưa đạt target): duration không bao giờ kết thúc (chỉ advance ở steady, coordinator.ts:451), E2 không fire (connectAttempts ngừng tăng — giá trị cũ trong tick cuối), E3 không fire (worker 'alive') → run kẹt 'running' MÃI + DB row kẹt 'running' (crash-detect chỉ khi server restart). Kịch bản 2: TẤT CẢ worker chết + restart fail (fork EMFILE) → `restart` delete khỏi map trước spawn (worker-farm.ts:165) → farm.total=0 → E3 guard `this.farm.total > 0` (coordinator.ts:483) short-circuit → kẹt. Fix: gọi checkHeartbeats định kỳ → restart worker treo; `workerTicks.delete(workerId)` trong onWorkerDied; đổi guard E3 thành `deathTimes.length > Math.max(1, total) * 0.5` |
| C-3 | E1 chạy 2 lần sau stop tay khi provisioning đang fail → phase 'stopped' bị ghi đè 'error' | MINOR | CONFIRMED | Thứ tự provisionAndLaunch (coordinator.ts:224-236): check E1 + check accounts<50% ĐỨNG TRƯỚC kill-switch guard (dòng 236). Stop tay lúc provisioning → finishRun('manual') xong (~20-100ms) → provisionAccounts (abort check mỗi account task, concurrency 25 — auth-factory.ts:182-183) trả summary SAU khi finishPromise kết thúc → failRate >50% → `finishRun('auto')` LẦN 2 (finishing đã reset false) → `endPhaseFromStop('auto')` → 'error' GHI ĐÈ 'stopped' + finalizeRun UPDATE lần 2 + runFinished đếm 2 + report file ghi đè. Fix: đưa kill-switch guard LÊN TRƯỚC 2 check E1 |
| C-4 | `DELETE /runs/:id` lúc run ĐANG CHẠY — FK CASCADE phá vĩnh viễn pipeline DB của run + vòng FK-fail | MINOR | CONFIRMED | `runDelete` (routes/history.ts:50-56) không guard trạng thái live; schema.sql:106/143: metric_samples + log_events FK `ON DELETE CASCADE` → xoá run giữa chừng = INSERT tick sau fail 23503 (business — không retry, store.ts:239-243) → batch re-queue (writer.ts:136-151) fail lại mỗi flush tới khi vượt cap 1000 rồi drop; `finalizeRun` cuối run UPDATE 0 row → status cuối không bao giờ ghi. Fix: reject DELETE khi status='running' (409) hoặc bỏ CASCADE |
| C-5 | Flush cuối lúc shutdown có thể skip batch khi flush timer đang in-flight + fail | MINOR | PLAUSIBLE | `flushTicks` (writer.ts:130-134) `if (this.flushing || !this.pendingTicks.length) return;` — SIGTERM đúng lúc flush 30s in-flight: `doWriteRunFinish`'s flush (writer.ts:108) return ngay; `shutdown`'s flush (writer.ts:64) return ngay nếu vẫn flushing; in-flight flush FAIL (transient) → batch re-queue (writer.ts:139) SAU khi cả 2 flush đã return → `pool.end()` (writer.ts:65) → batch mất. Double-fault (shutdown đúng lúc DB lỗi transient). Fix: flushTicks trả promise của in-flight flush (await cùng) |
| C-6 | `LOADTEST_REGISTER_RAMP`, `LOADTEST_FIXTURE_POST_IDS`, `LOADTEST_PROVISION_CONCURRENCY` có trong code nhưng THIẾU trong `.env.example` | MINOR | CONFIRMED | Grep đối chiếu 3 nguồn: config.ts:185 (REGISTER_RAMP) + config.ts:186 (FIXTURE_POST_IDS) đọc từ env, README có bảng (dòng 74-75) nhưng `loadtest/.env.example` KHÔNG có; `LOADTEST_PROVISION_CONCURRENCY` (auth-factory.ts:25) không có ở CẢ .env.example lẫn README. Dev không biết key tồn tại → dùng default âm thầm. Fix: thêm 3 key vào .env.example (+ README cho PROVISION_CONCURRENCY) |

**Xác nhận ĐÚNG / REFUTED (verified thật, không chỉ đọc)**:
- **Contract drift KHÔNG có** (spot-check 5 shape end-to-end): RunSummary/RunDetail/MetricTick ↔ api-mappers.ts:8-79 ↔ frontend src/types/loadtest.ts:230-250 — field-for-field khớp (kể cả `status` string, `poolSourceRunId`, `config/report` re-parse). Envelope `{success,data}` ↔ `unwrap` (loadtest-api.ts:99-103, api.ts:130-135). `/status` → `{runId, phase, startAt, elapsedSec, isRunning, stopReason, config, lastTick}` (routes/run.ts:82-89 + getRunSnapshot coordinator.ts:623-632) khớp `RunStatus` frontend. Poll 1s (app-shell.tsx:263) so sánh terminal `['finished','stopped','error']` (loadtest.store.ts:145) trùng `RunPhase` backend (types.ts:12-21) — KHÔNG nhầm 'running' vs phase. `/metrics?since` filter `t.ts > since` (routes/run.ts:95) + frontend `since=lastTick.ts` → không trùng tick. Health shape (health.ts:28-39) ⊇ frontend `{status}`.
- **Gateway change ĐÚNG contract**: chat client `auth: {token}` + `Authorization: Bearer` header (socket.ts:85-99) gửi ACCESS token; gateway đọc `handshake.auth?.token` → query → header (websocket.gateway.ts:147-150) → verify `ACCESS_TOKEN_SECRET` (websocket.gateway.ts:157-161) — cùng loại token 2 phía, không phải refresh-token. Loadtest socket-farm (socket-farm.ts:93-101) gửi access token test account — cùng cơ chế. Refresh-token body: client `{ refreshToken }` (api.ts:66-70) ↔ `RefreshTokenBodyDto.refreshToken` (refreshToken.dto.ts:3-10); response `{accessToken, refreshToken}` ↔ unwrap — khớp.
- **newRunId collision REFUTED**: `runSeq` tăng mỗi call (config.ts:366-373), `buildRunConfig` chỉ gọi từ `start()` — 2 run cùng ms cùng process có seq khác nhau (wrap 1296 sau 1296 run) — không cửa sổ trùng. Timestamp full (không slice) — B-4 giữ.
- **Double-stop / double-cleanup REFUTED**: stop lần 2 khi đang finish → await finishPromise (coordinator.ts:303-305); stop lúc 'stopped'/'error' → no-op; cooldown timeout double-fire đều check `phase === 'cooldown'` (coordinator.ts:324-328, 458-460). Cleanup: dry-run default + quét theo runId — idempotent (cleanup.ts).
- **tool-metrics counter process-correct**: workerRestarts tăng ở PARENT (worker-farm.ts:174 — WorkerFarm chạy trong coordinator process); apiErrors/runFinished/dbWriteFail/dbRetry đều parent-side (api-server.ts:256, coordinator.ts:548, store.ts:226/241/245/253); worker child KHÔNG import tool-metrics (grep socket-farm.ts/worker.ts: rỗng) → `/metrics` Prometheus đọc đúng state parent, không counter child vô hình.
- **JSONL logger single-writer giữ**: `workerEnv` ghi đè `LOGTEST_LOG_FILE:''` (worker-farm.ts:23-25) → child logger.ts:227 no sink; grep toàn bộ loadtest: MỌI fs.writeFile/appendFile khác (auth.ts:84, auth-factory.ts:356, config.ts:419, report.ts:307-309) đều parent-process-only (worker không chạm disk) → không file nào parent+child cùng ghi.
- **Shutdown B-2 fix GIỮ end-to-end**: SIGTERM → server.ts:59-80 → `api.closeConnections()` → `coordinator.stop(true)` — finishing thì await finishPromise (coordinator.ts:303-305), ngược lại finishRun AWAIT writeRunFinish (coordinator.ts:591-597, không còn fire-and-forget) → `dbWriter.shutdown()` await finalizePromise (writer.ts:62) → flushTicks → pool.end (writer.ts:63-65). Chuỗi await đủ — không promise bỏ quên giữa stop→finalize→flush→disconnect (ngoài C-5 double-fault).
...
## Feature review — Performance (Users + sort)

Scope: per-user action state (`markActionStart/End`), `toRow()`, worker/coordinator `queryUsers` global merge, `/users` route, UsersPage poll 2.5s. Reviewed at 10k users (budget: FEATURES-loadtest-tool.md tick 1s, histogram O(1), status poll 1s).

| Severity | Finding | Evidence (file:line) | Verdict | Fix |
|---|---|---|---|---|
| MEDIUM | `/users` re-transfers + re-sorts ALL filtered rows from every worker per poll (~3MB/10k rows, 72MB/min, ~4.3GB/hr GC churn); worker query is SYNCHRONOUS inside IPC handler → blocks worker scheduler/echo processing; coordinator merge/sort runs on the same event loop as the 1s aggregateTick + HTTP server. OK at 10k (~2-4% duty), breaks the tick budget at 50-100k (target users) | coordinator.ts:676-689, worker.ts:41-52, socket-farm.ts:733-758, server.ts:45-54 | ACCEPTABLE @10k; WARN @50k+ | Cache queryUsers in coordinator keyed (filter,phase,sortBy,sortDir) TTL ~1-2s; only re-query on TTL expiry or param change |
| MEDIUM | No cache/coalescing: N open dashboard clients → N× full 3MB IPC query + sort every 2.5s; worst case 3 clients ≈ 9MB/s per poll burst on the coordinator event loop | run.ts:104-117 | ACCEPTABLE 1-2 clients | Same TTL cache + optional `?cache=false` for manual refresh |
| LOW | Redundant sort: each worker sorts its full set (socket-farm.ts:756), coordinator re-sorts merged set (coordinator.ts:688) — per-worker sort is wasted CPU (slice 0/MAX is a no-op) | socket-farm.ts:756, coordinator.ts:688 | PASS (waste) | Workers skip sort; coordinator sorts merged set only |
| LOW | `localeCompare` in compareUsers for email/phase/currentAction — ~10-50x slower than plain compare; ~30-60ms per 10k-row email sort (ASCII-safe to use `<`/`>`) | users-sort.ts:63 | PASS | Compare ASCII strings with `<`/`>` (fallback localeCompare only if non-ASCII) |
| LOW | UsersPage poll 2.5s has no in-flight guard: a >2.5s response is overtaken by a newer poll; stale page-0 rows can overwrite fresh rows via `mergeFirstPage` | UsersPage.tsx:160-188, 207-209 | PASS @10k | Abort/ignore stale: track fetch seq or in-flight flag in the interval callback |
| INFO | Hot path: `markActionStart/End` = 1 `Date.now()` + 2-3 field writes (~50-100ns/action), no strings/arrays; `sendChat` = 2× `Date.now()` (pre-existing outbox sentAt + markActionStart) | socket-farm.ts:99-108, 257-267 | PASS — hot path untouched | — |
| INFO | `toRow()` = 1 × 13-field object alloc/row/query, no stringify (~200ns; 2ms/10k). Sort is in-place (no copy). New per-user fields ≈ 200B × 10k = 2MB | socket-farm.ts:395-411, users-sort.ts:68-70 | PASS | — |
| INFO | phaseCounts O(n)/query trivial; LiveDashboard already downsamples to 1800 pts (charts.tsx:42-47) | socket-farm.ts:752-754, charts.tsx:43 | PASS | — |

Hot path impact: none measurable (adds ~0.1µs/action to a pre-existing ~100µs action path). Budget honored at 10k; the global-merge /users poll is the first unbudgeted poll — cache needed before 50k+ targets or multi-client use.

## Feature review — Correctness (Users + action state + sort)

Scope: `markActionStart/End` placement, stuck-action paths, echo counting, `users-sort.ts` + coordinator global merge, `/users` route validation, 2-chiều types, UsersPage sort/filter race. Verified: `npm run loadtest:typecheck` PASS, `npm run loadtest:test` 316 PASS, `npm run test` 437 PASS.

| Severity | Finding | Evidence (file:line) | Verdict | Fix |
|---|---|---|---|---|
| MEDIUM | **currentAction STUCK sau MATCH_TIMEOUT**: phase → idle + cooldown 30s nhưng currentAction giữ 'chat' — dashboard hiện "Đang chat" cho user thực sự idle. Tương tự khi enqueue FAIL (THROTTLED → cooldown 900s): currentAction từ lần enqueue OK trước không được reset. Tự hết sau ~6s khi REST read override (chat profile) — display-only, không phá metrics | socket-farm.ts:213-219 (MATCH_TIMEOUT), 350-362 (enqueue fail), 366 (markActionStart chỉ ở enqueue OK) | CONFIRMED (display bug) | Gọi `markActionEnd('chat', …)` (hoặc reset 'idle') trên cả 2 path; fail path cũng nên set `lastActionMs` |
| MEDIUM | **Out-of-order race ở UsersPage**: không có request-seq/in-flight guard — response cũ (filter/sort cũ, poll cũ) resolve SAU response mới → `setRows`/`setTotal` ghi đè danh sách đang đúng filter/sort; `mergeFirstPage`/append tiếp tục trộn trang cũ. Cửa sổ thật: poll 2.5s + worker IPC timeout tới 3s (farm.queryUsers) | UsersPage.tsx:160-188 (fetchPage không guard), 191-216 (effect + interval gọi song song) | CONFIRMED | Monotonic requestId (ref) — bỏ qua response cũ hơn; hoặc AbortController cho request đang bay khi param đổi |
| LOW | **Echo #1 kết thúc action khi msg #2 còn pending**: 2 msg in-flight (CHAT_SEND_MIN 2s < echo latency có thể lâu) → echo đầu tiên `markActionEnd('chat')` → currentAction='idle' trong khi outboxPending=1; `markActionStart` ở sendChat #2 cũng reset lastActionAt → lastActionAt/echo latency đo từ msg #2 không phải msg đang echo | socket-farm.ts:172-183 (echo handler), 262 (markActionStart mỗi send) | CONFIRMED (hiển thị sai ngắn, tự sửa khi send tiếp) | End chỉ khi outbox rỗng (hoặc track pendingCount/action); lastActionAt = sentAt msg đầu tiên còn pending |
| LOW | **Worker chết → stall 3s + total thiếu**: farm.queryUsers timeout trả `{rows:[], total:0}` cho worker chết (trong lúc chờ restart 2s) → mỗi poll /users trễ 3s và total giảm tạm thời | worker-farm.ts:210-213 (timeout resolve 0), coordinator.ts:676-689 (lặp cả worker chết) | CONFIRMED (transient, self-heal khi restart) | Coordinator bỏ qua worker `!alive` trong vòng lặp; count total từ worker alive |
| LOW | **messagesSent/messagesEchoed/lastActionMs không hiển thị ở UsersPage** — field mới thu thập qua API nhưng không có cột/detail dialog; không thể kiểm chứng qua UI | UsersPage.tsx:53 (GRID_COLS 10 cột), 65-131 (UserRow không render 3 field) | CONFIRMED (gap feature-completeness) | Thêm cột hoặc user-detail dialog |
| PASS | Sort: whitelist cứng + switch tĩnh (injection-safe, `__proto__`/SQL đều fallback index), null luôn cuối CẢ asc lẫn desc, stable (V8 sort + input theo index), sortDir lạ → asc — khớp triết lý lenient của route clamp offset/limit | users-sort.ts:27-70, tests users-sort.test.ts:28-124 | PASS | — |
| PASS | Coordinator merge: fetch ALL rows mọi worker (offset 0, limit MAX) → merge → sort global → slice — pagination đúng kể cả offset > 0; `total` = tổng filtered (không bị sort ảnh hưởng); per-worker limit không phá page 2+ | coordinator.ts:667-690 | PASS | — |
| PASS | Route /users: phase whitelist 8 phase (lạ → bỏ qua), sortBy/sortDir whitelist (lạ → default), offset/limit clamp — nhất quán lenient; response echo sortBy/sortDir đã normalize | run.ts:104-117 | PASS | — |
| PASS | Types 2-chiều: VirtualUserRow có đủ 5 field mới + UserPhase/UserActionState 2 bên; typecheck file phủ cả 3 (compile-time), UserSortField frontend khớp 7 field whitelist | src/types/loadtest.ts:40-64, types-contract.typecheck.ts:73-89 | PASS | — |
| PASS | Echo counting đúng: `messagesEchoed++` chỉ khi clientMsgId khớp pending; no-echo sau prune (TTL 60s) không đếm trùng (đã tính NO_ECHO fail); leaveRoom → idle; typing start+end synchronous (không flash 'typing' khi toRow) | socket-farm.ts:172-183, 200-208, 269-275, 278-286 | PASS | — |
| PASS | REST/topic failure path: RestDriver không bao giờ reject (requestJson catch mọi lỗi → NETWORK fail) → markActionEnd luôn chạy, không stuck vì exception | http.ts:110-124, rest-actions.ts:83-99 | PASS | — |

Verdict: 2 MEDIUM thật sự (stuck action ở timeout/fail path; race out-of-order frontend) — đều display-only, không phá metrics/DB; 3 LOW; sort/merge/route/types/tests đúng. Chốt: OK để merge sau khi fix 2 MEDIUM (thay đổi < 10 dòng) hoặc chấp nhận như known-issue ở CỔNG 3.

## Feature review — UI/UX (Users page + charts)

Phạm vi: UsersPage (bảng virtualized + sort/filter + donut), chart revamp (phase-donut, charts.tsx, chart-card, chart-theme), LiveDashboardPage, App route, app-shell nav, fmtRelative, loadtest-api.users(). Verifier: `npm run build` PASS, `npm run test` 437/437 PASS (UsersPage.test.tsx 5 tests không phát hiện lỗi căn cột). Backend contract đã đối chiếu (routes/run.ts users handler, users-sort.ts, socket-farm.queryUsers).

| Severity | Finding | Evidence | Verdict | Fix |
|---|---|---|---|---|
| HIGH | **Lệch cột 5–9 giữa header và row** — header: `# Email Phase Action [Reconnect, Outbox, Hoạt động, Room, Socket] Lỗi gần nhất`; row render: `index email phase action [roomId, socket, reconnectCount, outboxPending, lastActionAt] lastError`. User nhìn thấy roomId dưới header "Reconnect", icon socket dưới "Outbox", reconnectCount dưới "Hoạt động", outboxPending dưới "Room", lastActionAt dưới "Socket". Phá luôn mục tiêu chính "xem tài khoản đang làm gì" (số reconnect/outbox/hoạt động gần nhất đọc sai cột). Test hiện tại không assert header↔cell nên không bắt được. | `src/pages/loadtest/UsersPage.tsx:349-371` (header) vs `:74-127` (UserRow); `SORTABLE` :43-51 | CONFIRMED | Sắp lại span trong UserRow theo đúng thứ tự header: reconnectCount → outboxPending → lastActionAt → roomId → socket (hoặc đổi SORTABLE). |
| MEDIUM | **Tooltip latency hiển thị unit trùng "120ms ms"** — `format={fmtMs}` (đã thêm "ms") + `unit="ms"` (thêm nữa) → "120ms ms". | `src/components/loadtest/charts.tsx:360` | CONFIRMED | Bỏ `unit="ms"` (giữ format={fmtMs}). |
| MEDIUM | **Không có error state khi /users fail** — catch nuốt im lặng (:181-185); lần đầu mount + server offline → bảng trống hiện "Không có user phù hợp bộ lọc" (:388) hoặc "đang provisioning/connect..." (:388) — gây hiểu nhầm là không có data/filter sai thay vì lỗi mạng. | `src/pages/loadtest/UsersPage.tsx:181-185, 386-389` | CONFIRMED | Thêm state `fetchError` + AlertBanner "Không tải được danh sách user — kiểm tra server 3401". |
| MEDIUM | **Race response khi đổi sort/filter nhanh hoặc poll trùng** — không có sequence token: response cũ (sort cũ) resolve sau response mới → `setRows` ghi đè bảng sai thứ tự; poll in-flight (`append=true`, mergeFirstPage :60-63) trả page cũ sau reset → `prev.length <= fresh.length → return fresh` :62 ghi đè bảng = toàn bộ row sai sort tới 2.5s. Worker fan-out 10k user làm window hiện thực. | `src/pages/loadtest/UsersPage.tsx:160-195, 207-209` | CONFIRMED | Sequence token (requestId tăng dần, bỏ response cũ) hoặc AbortController. |
| MEDIUM | **Mobile (<lg): header không cuộn ngang cùng body** — GRID_COLS cố định ~586px (56+112+104+60+80+64+110) > viewport 375px; body scroll container `overflow-auto` (chỉ body cuộn), header là sibling ngoài container (:344-372 vs :373-409) → cuộn ngang là header/body lệch cột ngay; không có affordance scroll ngang. | `src/pages/loadtest/UsersPage.tsx:53, 344-409` | CONFIRMED | Đưa header vào trong scroll container (sticky top-0 trong đó), hoặc overflow-x-auto chung. |
| LOW | **Sort/filter reset scroll về đầu** — reset effect `setRows([])` (:191-195) → skeleton 8 dòng → browser clamp scrollTop về ~0; sau sort trên bảng 10k row user rơi về đầu bảng, mất context. | `src/pages/loadtest/UsersPage.tsx:191-195, 380-389` | CONFIRMED | Chấp nhận được cho sort server-side nhưng nên restore scroll hoặc giữ nửa trang cuối khi refetch. |
| LOW | **A11y bảng: aria-sort đặt trên `<button>` (chỉ hợp lệ trên columnheader/gridcell) + thiếu role cell/columnheader** — `role="table"` > `role="row"` > span thường (không role="cell"); SR không đọc được cấu trúc bảng/sort state đáng tin. Keyboard vẫn dùng được (button focusable). | `src/pages/loadtest/UsersPage.tsx:352-363, 72-128` | CONFIRMED | role="columnheader" + aria-sort trên th/cell; hoặc dùng grid. |
| LOW | **Contrast badge phase < WCAG AA** — badgeStyle (:55-57) dùng hue phase làm text 12px (text-xs) trên nền tint 15%: chart-1 (38 92% 60%) ≈ 1.8:1, chart-3 ≈ 2.6:1 — dưới 4.5:1 cho text nhỏ. Có nhãn chữ (không color-only) nhưng kém đọc. | `src/components/loadtest/chart-theme.ts:19-28`, `src/pages/loadtest/UsersPage.tsx:55-57, 80-84` | CONFIRMED (tính toán) | Text phase dùng biến thể tối hơn (vd `foreground` mix) hoặc tăng cỡ/bold. |
| LOW | **fmtRelative không tự nhiên tiếng Việt** — "5s trước"/"3m trước" ("m" lẫn phút/mét); chuẩn là "5 giây trước"/"3 phút trước". Ngoài ra khi run idle không có re-render → giá trị "X trước" đứng yên tới khi thao tác. | `src/lib/loadtest-format.ts:58-65` | CONFIRMED | Đổi "giây/phút/giờ"; thêm interval 30s hiển thị lại khi idle (tùy chọn). |

Đã verify KHÔNG lỗi (REFUTED): `vote_kick` thiếu trong ACTION_STATE_LABELS — REFUTED (backend `UserActionState` = union y hệt, types-contract.typecheck enforce 2 chiều); phaseCounts donut theo filter — REFUTED (backend đếm toàn bộ user, socket-farm.ts:752-754); `phase` param — dùng đúng (whitelist run.ts:109-110); double-interval poll — có guard `pollRef`; poll dừng khi run kết thúc (isRunning=false từ server); donut empty/zero state có; PHASE_COLORS nhất quán UsersPage + LiveDashboard (cùng chart-theme.ts); LiveDashboard không đổi field cũ (types không đổi, build+437 tests PASS); layout card grid không overflow desktop, donut+legend vừa khối h-44/h-56.

Verdict: 1 HIGH (lệch cột — fix < 5 dòng, chặn CỔNG 3), 3 MEDIUM (unit trùng, thiếu error state, race sort/poll), 4 LOW. Build/test xanh. Cần fix HIGH + 2 MEDIUM dễ (tooltip unit, error state) trước merge; race và mobile-scroll ghi nhận known-issue.
