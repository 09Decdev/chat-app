-- =====================================================================
-- MAYogu LoadTest Tool — PostgreSQL schema (v1)
-- -------------------------------------------------------------
-- Nguồn thiết kế: docs/PRD-loadtest-run-database.md §2.3 (bảng Run,
-- MetricSample, Pool, PoolAccount, LogEvent) + PRD-loadtest-admin-auth.md
-- (bảng admin_users cho xác thực dashboard).
--
-- Quy ước:
--   - Tên bảng/cột: snake_case (SQL convention). Tên cột PRD cũ (camelCase)
--     được ghi chú dạng "= PRD-old: runId" ở mỗi cột để đồng nhất.
--   - JSON payload lưu TEXT — không bảng con.
--   - Chạy trên instance postgres-loadtest (port 5439), db `loadtest`.
-- =====================================================================

-- Migration version (Module D2 — auto-migration đơn giản, không framework)
